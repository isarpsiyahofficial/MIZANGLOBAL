# MİZAN GLOBAL — 29 Kullanıcı Dili İçin Ülke Kapsamı v2

Bu belge, MİZAN GLOBAL ilk sürümünde ülke/bölge seçim ekranına alınacak ana ülke kapsamını sabitler.

## Kapsam ölçütü

Bir ülke, desteklenen kullanıcı dillerinden en az birinin ülke çapında resmî, ortak resmî, fiilî ana çalışma dili veya yaygın merkezi yönetim/dijital hizmet dili olması hâlinde ana kapsama alınır. Yalnız küçük azınlık topluluklarının konuştuğu diller ülkeyi otomatik olarak kapsama almaz.

Ana kapsamda 161 ülke/devlet düzeyi pazar vardır. Hong Kong, Makao, Porto Riko, Aruba, Curaçao, Sint Maarten ve benzeri özel statülü bölgeler bu sayıya katılmaz; ayrı `ülke/bölge` kayıtları olarak ikinci tabloda değerlendirilecektir.

Basitleştirilmiş Çince ve Geleneksel Çince kullanıcıya iki ayrı dil seçeneği olarak gösterilmeyecektir. Dil seçim ekranında yalnız **Çince** bulunacaktır. Gerekli yazı sistemi ülke/bölge veya cihaz tercihi üzerinden uygulama içinde otomatik yönetilecektir. Bu birleşme nedeniyle mevcut görünür kullanıcı dili sayısı 29'dur.

## 29 kullanıcı dili ve kapsadığı ülkeler

### 1. Türkçe (`tr`)
Türkiye, Kıbrıs.

### 2. İngilizce (`en`)
Amerika Birleşik Devletleri, Hindistan, Pakistan, Nijerya, Filipinler, Birleşik Krallık, Tanzanya, Kanada, Sudan, Avustralya, Güney Afrika, Malavi, Kamerun, Kenya, Gana, Zimbabve, İrlanda, Yeni Zelanda, Papua Yeni Gine, Singapur, Liberya, Eritre, Güney Sudan, Zambiya, Sierra Leone, Jamaika, Ruanda, Botsvana, Uganda, Trinidad ve Tobago, Gambiya, Mauritius, Esvatini, Fiji, Guyana, Solomon Adaları, Lesotho, Malta, Bahamalar, Belize, Barbados, Vanuatu, Namibya, Saint Lucia, Kiribati, Grenada, Saint Vincent ve Grenadinler, Antigua ve Barbuda, Marshall Adaları, Dominika, Mikronezya Federal Devletleri, Saint Kitts ve Nevis, Seyşeller, Tonga, Nauru, Burundi, Samoa, Palau, Tuvalu.

### 3. İspanyolca (`es`)
Meksika, Arjantin, İspanya, Kolombiya, Venezuela, Peru, Şili, Ekvador, Guatemala, Küba, Honduras, Dominik Cumhuriyeti, Bolivya, El Salvador, Nikaragua, Paraguay, Kosta Rika, Panama, Uruguay, Ekvator Ginesi.

### 4. Brezilya Portekizcesi (`pt-BR`)
Brezilya.

### 5. Avrupa Portekizcesi (`pt-PT`)
Portekiz, Angola, Mozambik, Gine-Bissau, Doğu Timor, Cabo Verde, São Tomé ve Príncipe, Ekvator Ginesi.

### 6. Fransızca (`fr`)
Fransa, Demokratik Kongo Cumhuriyeti, Kamerun, Cezayir, Fas, Kanada, Fildişi Sahili, Burundi, Madagaskar, Tunus, Burkina Faso, Benin, Haiti, Senegal, Belçika, Gine, Mali, Kongo Cumhuriyeti, Togo, İsviçre, Nijer, Çad, Orta Afrika Cumhuriyeti, Gabon, Mauritius, Ruanda, Lüksemburg, Ekvator Ginesi, Cibuti, Komorlar, Vanuatu, Seyşeller, Monako.

### 7. Almanca (`de`)
Almanya, Avusturya, İsviçre, Belçika, Lüksemburg, Lihtenştayn.

### 8. İtalyanca (`it`)
İtalya, İsviçre, San Marino, Vatikan.

### 9. Hollandaca (`nl`)
Hollanda, Belçika, Surinam.

### 10. Lehçe (`pl`)
Polonya.

### 11. Romence (`ro`)
Romanya, Moldova.

### 12. Yunanca (`el`)
Yunanistan, Kıbrıs.

### 13. Rusça (`ru`)
Rusya, Kazakistan, Belarus, Kırgızistan.

### 14. Ukraynaca (`uk`)
Ukrayna.

### 15. Arapça (`ar`)
Mısır, Suudi Arabistan, Cezayir, Sudan, Irak, Yemen, Fas, Suriye, Ürdün, Tunus, Birleşik Arap Emirlikleri, Libya, Filistin, Lübnan, Somali, Moritanya, Çad, Umman, Kuveyt, Katar, Bahreyn, Komorlar, Cibuti, Eritre.

### 16. Farsça (`fa`)
İran, Afganistan. Afganistan için Dari kullanımı ayrı ülke varyantı olarak ana dil seviyesinde hazırlanacaktır.

### 17. İbranice (`he`)
İsrail.

### 18. Hintçe (`hi`)
Hindistan.

### 19. Bengalce (`bn`)
Bangladeş, Hindistan.

### 20. Urduca (`ur`)
Pakistan, Hindistan.

### 21. Endonezce (`id`)
Endonezya.

### 22. Malayca (`ms`)
Malezya, Singapur, Brunei.

### 23. Filipince (`fil`)
Filipinler.

### 24. Vietnamca (`vi`)
Vietnam.

### 25. Tayca (`th`)
Tayland.

### 26. Svahili (`sw`)
Tanzanya, Kenya, Uganda.

### 27. Çince (`zh`)
Çin, Singapur, Tayvan. Dil seçim ekranında tek seçenek olarak gösterilir. Çin ve Singapur için sadeleştirilmiş, Tayvan için geleneksel yazı sistemi uygulama tarafından otomatik seçilir.

### 28. Japonca (`ja`)
Japonya.

### 29. Korece (`ko`)
Güney Kore, Kuzey Kore.

## Özel statülü bölgeler

Aşağıdaki yerler ana 161 sayısına katılmaz; farklı para birimi, mevzuat veya yerel biçim ihtiyacı nedeniyle ayrı `ülke/bölge` kaydı olarak değerlendirilecektir:

- Hong Kong: Çince, İngilizce
- Makao: Çince, Avrupa Portekizcesi
- Porto Riko: İspanyolca, İngilizce
- Aruba: Hollandaca
- Curaçao: Hollandaca
- Sint Maarten: Hollandaca, İngilizce
- Karayip Hollandası: Hollandaca, İngilizce
- Batı Sahra: Arapça; statüsü açık biçimde bölge olarak gösterilecek

## Çince yazı sistemi davranışı

- Kullanıcı dil seçim ekranında yalnız `Çince` görecektir.
- Ayrı `Basitleştirilmiş Çince` ve `Geleneksel Çince` seçenekleri bulunmayacaktır.
- Çin ve Singapur seçildiğinde sadeleştirilmiş yazı sistemi kullanılacaktır.
- Tayvan, Hong Kong veya Makao seçildiğinde geleneksel yazı sistemi kullanılacaktır.
- Çince dili başka bir ülkeyle birlikte seçilmişse önce cihazın Çince yazı sistemi tercihi kullanılacak; cihaz tercihi yoksa güvenli varsayılan olarak sadeleştirilmiş yazı sistemi kullanılacaktır.
- Yazı sistemi farkı arayüzde ikinci bir dil gibi gösterilmeyecek ve dil sayısını artırmayacaktır.

## Uygulama kuralları

- Ülke seçimi dili veya para birimini zorla değiştirmeyecek.
- Her ülke için varsayılan ve alternatif desteklenen diller ayrı tutulacak.
- Her ülkenin para birimi, tarih/sayı biçimi, hafta başlangıcı, ödeme sıklığı ve raporlama dönemleri ayrı kaynaklarla doğrulanacak.
- Ülke listesine girmek, o ülkeye tek bir zorunlu borç dönemi atanacağı anlamına gelmez.
- Kullanıcının gerçek vade tarihi ve seçtiği ödeme sıklığı hesaplamada esas olacaktır.
- Rusça, Bengalce, Urduca, Svahili ve benzeri bölgesel/ortak kullanım dilleri yalnız ilgili ülkelerde kullanıcı seçimi olarak sunulacak; ülkenin varsayılan dili yanlış değiştirilmeyecek.
- Çin, Tayvan, Hong Kong ve Makao ayrı ülke/bölge kayıtları olarak kalacak; ancak hepsi kullanıcıya tek `Çince` dil seçeneği üzerinden hizmet verecektir.

## Ana kaynaklar

- Unicode CLDR Language–Territory Information: https://unicode.org/cldr/charts/49/supplemental/language_territory_information.html
- Unicode CLDR resmî dil tanımları: https://cldr.unicode.org/index/cldr-spec/definitions
- ISO 3166 kapsam açıklaması: https://www.iso.org/iso-3166-country-codes.html
- Birleşmiş Milletler üye devlet kapsamı: https://www.un.org/en/about-us/member-states

Bu liste kodlamadan önce ülke bazında ikinci kaynak ve resmî ülke kaynağıyla tekrar doğrulanacaktır; kaynak çelişkisi çözülmeden veri üretime alınmayacaktır.
