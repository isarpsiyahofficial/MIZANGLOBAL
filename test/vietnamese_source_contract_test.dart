...
void main() {
  test('Vietnamese owns the complete 791-key static contract', () {
    ...
    expect(mizanVietnamese.values.every((value) => value.trim().isNotEmpty), isTrue);
  });
  ...
  test('Vietnamese dynamic grammar localizes counts and due status', () {
    expect(
      translateVietnameseReviewedDynamic('3 gün kaldı', (value) => mizanVietnamese[value] ?? value),
      'Còn 3 ngày',
    );
    expect(
      translateVietnameseReviewedDynamic('Ödeme 2 gün gecikti.', (value) => mizanVietnamese[value] ?? value),
      'Khoản thanh toán đã quá hạn 2 ngày.',
    );
    expect(
      translateVietnameseReviewedDynamic('Ödeme hatırlatması 3', (value) => mizanVietnamese[value] ?? value),
      'Lời nhắc thanh toán 3',
    );
  });

  test('Vietnamese copy rejects neighboring-language and CJK leakage in core navigation', () {
    final joined = <String>[
