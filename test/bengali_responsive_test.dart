...
Future<void> _visitBengaliTabs(WidgetTester tester) async {
...
void main() {
  testWidgets('Bengali 320x568 at 1.4x is LTR and avoids overflow', (
    tester,
  ) async {
    await _pumpBengaliAt(tester, const Size(320, 568), textScale: 1.4);
    ...
    final label = find.text('হোম').first;
    expect(Directionality.of(tester.element(label)), TextDirection.ltr);

    await _visitBengaliTabs(tester);
    await _disposeApp(tester);
  });

  testWidgets('Bengali 412x915 at 2.0x remains LTR without overflow', (
    tester,
  ) async {
    await _pumpBengaliAt(tester, const Size(412, 915), textScale: 2);
    expect(
      Directionality.of(tester.element(find.text('হোম').first)),
      TextDirection.ltr,
    );
    await _visitBengaliTabs(tester);
    await _disposeApp(tester);
