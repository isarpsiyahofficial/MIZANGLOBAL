...
Future<void> _pump(WidgetTester tester,Size size,double scale)async{
  ...
  final state=comprehensiveState(reference:DateTime(2026,8,7)).copyWith(appLanguageTag:'ms',debtRegionCountryCode:'MY',defaultCurrencyCode:'MYR');
...
void main(){
  testWidgets('Malay 320x568 at 1.4x has no overflow or Indonesian/Turkish nav leakage',(tester)async{
    await _pump(tester,const Size(320,568),1.4);
    for(final text in const['Laman utama','Rekod','Perbelanjaan','Laporan','Tetapan'])expect(find.text(text),findsWidgets);
    expect(find.text('Ana sayfa'),findsNothing);expect(find.text('Beranda'),findsNothing);expect(Directionality.of(tester.element(find.text('Laman utama').first)),TextDirection.ltr);await _visit(tester);
  });
  testWidgets('Malay 412x915 at 2.0x remains usable without overflow',(tester)async{await _pump(tester,const Size(412,915),2);expect(Directionality.of(tester.element(find.text('Laman utama').first)),TextDirection.ltr);await _visit(tester);});
}
