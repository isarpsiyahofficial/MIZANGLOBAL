...
Map<String, dynamic> _stateJson({required bool explicitCurrencies}) => <String, dynamic>{
  'schemaVersion': 13,
  'setupCompleted': true,
  'appLanguageTag': 'en',
...
void main() {
  test('legacy globalization-era records without per-record currency freeze to persisted default once', () {
  ...
  test('modern records preserve independent ISO currencies through JSON round trip', () {
    final original = MizanState.fromJson(_stateJson(explicitCurrencies: true));
  ...
  test('changing main default currency never rewrites historical record currencies or amounts', () {
  ...
  test('remaining totals stay in separate currency buckets', () {
    ...
    expect(totals.keys, containsAll(<String>['EUR','GBP','CHF','JPY','TRY']));
