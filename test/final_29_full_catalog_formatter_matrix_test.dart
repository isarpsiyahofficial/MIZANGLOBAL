...
void main() {
  ...
  test(
    'all 29 languages cover all 154 currencies and 161 countries offline',
    () async {
      ...
      for (final language in authoritativeLanguages) {
        for (final currency in catalog.currencies) {
          ...
          expect(
            ...
            isTrue,
            reason:
                '$language/${currency.code} exact ISO lookup must resolve itself',
          );
        }
        ...
        for (final country in catalog.countries) {
          ...
          expect(
            ...
            isTrue,
            reason:
                '$language/${country.code} must be searchable by country code',
          );
        }
      }
    },
  ...
  test(
    '29 x 154 money formatting is finite, nonempty, and round-trips',
    () async {
      final catalog = await GlobalCatalogRepository.load();
