...
void main(){
  ...
  test('Filipino reports keep report PDF language currency and user data isolated',(){
    final now=DateTime(2026,8,7,12);final state=comprehensiveState(reference:now).copyWith(appLanguageTag:'fil',debtRegionCountryCode:'PH',defaultCurrencyCode:'PHP');
    ...
    expect(report.languageTag,'fil');expect(report.currencyCode,'PHP');expect(report.filter.period.label,'Buwanan');expect(report.range.label,'Agosto 2026');
    expect(report.selectedPersonNames.any((v)=>v.contains('İbrahim')),isTrue);
    for(final leak in const['Ağustos','Agustus','Ogos','August','8월','8月'])expect(report.range.label,isNot(contains(leak)),reason:leak);
    expect(MizanI18n.text('PDF raporu'),'PDF report');expect(MizanI18n.text('Kalan ödeme yükü'),'Natitirang obligasyon sa pagbabayad');
  });

  test('Filipino reminders localize system copy while custom user message stays unchanged',(){
