...
void main(){
  final maps=<String,Map<String,String>>{
    'es':mizanSpanish,'pt-BR':mizanPortugueseBr,'pt-PT':mizanPortuguesePt,'fr':mizanFrench,'de':mizanGerman,'it':mizanItalian,'nl':mizanDutch,'pl':mizanPolish,'ro':mizanRomanian,'el':mizanGreek,'ru':mizanRussian,'uk':mizanUkrainian,'ar':mizanArabic,'fa':mizanPersian,'he':mizanHebrew,'hi':mizanHindi,'bn':mizanBengali,'ur':mizanUrdu,'id':mizanIndonesian,'ms':mizanMalay,'fil':mizanFilipino,'ko':mizanKorean,'ja':mizanJapanese,'zh':mizanChinese,
  };
  test('all 24 translated static maps have exactly the same 791 stable Turkish source keys',(){final reference=mizanIndonesian.keys.toSet();expect(reference,hasLength(791));for(final entry in maps.entries){expect(entry.value,hasLength(791),reason:entry.key);expect(entry.value.keys.toSet(),reference,reason:'key mismatch: ${entry.key}');expect(entry.value.values.every((v)=>v.trim().isNotEmpty),isTrue,reason:'empty value: ${entry.key}');}});
  test('CJK static system copy never crosses Korean Japanese Chinese script boundaries',(){final ko=mizanKorean.values.join('\n');final ja=mizanJapanese.values.join('\n');final zh=mizanChinese.values.join('\n');expect(RegExp(r'[\u3040-\u30ff]').hasMatch(ko),isFalse);expect(RegExp(r'[\uac00-\ud7af]').hasMatch(ja),isFalse);expect(RegExp(r'[\uac00-\ud7af\u3040-\u30ff]').hasMatch(zh),isFalse);for(final value in const['首页','报告','设置'])expect(ko,isNot(contains(value)));for(final value in const['홈','보고서','설정']){expect(ja,isNot(contains(value)));expect(zh,isNot(contains(value)));}for(final value in const['ホーム','レポート','設定']){expect(ko,isNot(contains(value)));expect(zh,isNot(contains(value)));}});
}
