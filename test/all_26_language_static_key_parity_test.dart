...
void main() {
  final maps = <String,Map<String,String>>{
    'es':mizanSpanish,'pt-BR':mizanPortugueseBr,'pt-PT':mizanPortuguesePt,'fr':mizanFrench,'de':mizanGerman,'it':mizanItalian,'nl':mizanDutch,'pl':mizanPolish,'ro':mizanRomanian,'el':mizanGreek,'ru':mizanRussian,'uk':mizanUkrainian,'ar':mizanArabic,'fa':mizanPersian,'he':mizanHebrew,'hi':mizanHindi,'bn':mizanBengali,'ur':mizanUrdu,'id':mizanIndonesian,'ms':mizanMalay,'fil':mizanFilipino,'vi':mizanVietnamese,'th':mizanThai,'sw':mizanSwahili,'zh':mizanChinese,'ja':mizanJapanese,'ko':mizanKorean,
  };

  test('all 27 translated static maps have exactly the same 791 stable Turkish source keys', () {
    ...
    for (final entry in maps.entries) {
      ...
      expect(entry.value.keys.toSet(), reference, reason: 'key mismatch: ${entry.key}');
      expect(entry.value.values.every((v) => v.trim().isNotEmpty), isTrue, reason: 'empty value: ${entry.key}');
    }
  });

  test('CJK static system copy never crosses Korean Japanese Chinese script boundaries', () {
    ...
    for (final value in const ['首页','报告','设置']) expect(ko, isNot(contains(value)));
  ...
  test('Thai critical static copy is Thai and Vietnamese Swahili stay outside Thai script', () {
