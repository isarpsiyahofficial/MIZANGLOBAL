...
void main() {
  ...
  test(
    'Malay money numbers and Gregorian dates follow Malaysia conventions',
    () {
  ...
  test(
    'Malay dynamic payment states do not fall back to Turkish or Indonesian',
    () {
      MizanI18n.setProfile(languageTag: 'ms', currencyCode: 'MYR');
      ...
      expect(
        ...
        isNot(contains('Pembayaran terlambat')),
      );
  ...
  test('Malay catalogs are complete offline and native-searchable', () async {
    ...
    expect(
      catalog.countries.every((item) => item.nameFor('ms').trim().isNotEmpty),
      isTrue,
    );
    expect(
      catalog.currencies.every((item) => item.nameFor('ms').trim().isNotEmpty),
      isTrue,
    );
    expect(
      catalog.currencies
          .where((item) => item.matches('ringgit'))
          .any((item) => item.code == 'MYR'),
      isTrue,
    );
  });
  ...
  test('Malay report PDF notification and backup vocabulary is separated', () {
    ...
    expect(
      MizanI18n.text('Günlük gider hatırlatmaları'),
      'Peringatan perbelanjaan harian',
    );
  });
