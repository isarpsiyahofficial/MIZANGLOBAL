...
void main() {
  testWidgets('Chinese 320x568 at 1.4x has no Korean or Japanese nav leakage', (
    t,
  ) async {
    await _pump(t, const Size(320, 568), 1.4);
    for (final text in const ['首页', '记录', '支出', '报告', 'PRO'])
    ...
    expect(
      Directionality.of(t.element(find.text('首页').first)),
      TextDirection.ltr,
    );
    await _visit(t);
  });
  testWidgets('Chinese 412x915 at 2.0x remains usable without overflow', (
    t,
  ) async {
