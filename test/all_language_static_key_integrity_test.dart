...
void main() {
  ...
  test('English covers the same 791 source keys without empty fallback', () {
    ...
    for (final key in reference) {
      final value = MizanI18n.text(key);
      values.add(value);
      if (value != key) changed++;
    }
    expect(values, hasLength(791));
    expect(values.every((value) => value.trim().isNotEmpty), isTrue);
    expect(changed, greaterThan(740));
  });

  test(
    'Turkish stable source remains the reference and is never rewritten',
    () {
