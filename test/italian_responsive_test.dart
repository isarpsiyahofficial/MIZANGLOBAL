...
Future<void> _visitItalianTabs(WidgetTester tester) async {
...
void main() {
  testWidgets('Italian 320x568 at 1.4x renders all tabs without overflow', (
    tester,
  ) async {
    await _pumpItalianAt(tester, const Size(320, 568), textScale: 1.4);
    expect(find.text('Panoramica'), findsWidgets);
  ...
  testWidgets('Italian 412x915 at 2.0x renders all tabs without overflow', (
    tester,
  ) async {
    await _pumpItalianAt(tester, const Size(412, 915), textScale: 2);
    await _visitItalianTabs(tester);
    await _disposeApp(tester);
  });
}
