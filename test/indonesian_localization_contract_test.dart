...
void main() {
  ...
  test('Indonesian copy contains no foreign-script or Turkish fallback leakage', () {
    ...
    for (final entry in mizanIndonesian.entries) {
      ...
      expect(turkishFallback.hasMatch(entry.value), isFalse, reason: entry.key);
    }
  });

  test('id and legacy in tags normalize to Indonesian without affecting other languages', () {
  ...
  test('dynamic Indonesian grammar remains natural and preserves user data', () {
    String translate(String source) => mizanIndonesian[source] ?? source;
    expect(translateIndonesianReviewedDynamic('3 gün kaldı', translate), '3 hari lagi');
    expect(
