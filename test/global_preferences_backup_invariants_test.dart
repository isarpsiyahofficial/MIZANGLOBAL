...
Map<String, dynamic> _businessPayload(MizanState state) => <String, dynamic>{
  ...
  'expenseCategories': state.expenseCategories.map((item) => item.toJson()).toList(),
  'expenses': state.expenses.map((item) => item.toJson()).toList(),
  'incomes': state.incomes.map((item) => item.toJson()).toList(),
  'notificationSlots': state.notificationSlots.map((item) => item.toJson()).toList(),
  'paymentNotificationSlots': state.paymentNotificationSlots.map((item) => item.toJson()).toList(),
...
void main() {
  test('language change preserves region, default currency, and all business data', () async {
  ...
  test('region change preserves language, default currency, and all business data', () async {
    final initial = comprehensiveState().copyWith(
      setupCompleted: true,
      appLanguageTag: 'de',
  ...
  test('default currency change preserves language, region, and all business data', () async {
    final initial = comprehensiveState().copyWith(
      setupCompleted: true,
      appLanguageTag: 'en',
  ...
  test('current CSV snapshot carries and restores all three profile preferences', () {
  ...
  test('legacy Turkish state without global profile remains compatible and data-identical', () {
  ...
  test('backup merge adopts profile only for an unfinished fresh setup', () {
    ...
    final merged = service.mergeStates(MizanState.freshInstall(), imported).state;
  ...
  test('backup merge into configured app preserves current profile preferences', () {
    const service = CsvBackupService();
