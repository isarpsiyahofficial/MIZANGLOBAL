...
MizanState _stateFor(_LocaleCase locale) {
  return comprehensiveState(reference: _now, currencyCode: locale.currency)
      .copyWith(
        setupCompleted: true,
...
Future<void> _openAndCloseDialog(
  ...
  for (final source in requiredSourceCopy) {
    ...
    expect(
      find.text(localized),
      findsWidgets,
      reason: '${locale.tag}: missing localized dialog copy for "$source" => "$localized"',
    );
  }
  expect(
...
void main() {
  ...
  test('${locale.tag}: runtime, report, reminder, backup and destructive flows', () async {
    MizanClock.setNowForTesting(_now);
    ...
    for (final marker in const ['İbrahim', 'Bank 24', '東京', 'Việt Nam', 'M-Pesa']) {
      expect(visibleUserCopy, contains(marker), reason: '${locale.tag}: $marker');
    }
    expect(visibleUserCopy, isNot(contains('\u{E000}')), reason: locale.tag);

    final formatted = money(1234.56, currencyCode: locale.currency);
    expect(formatted.trim(), isNotEmpty, reason: '${locale.tag}: money formatter');
    ...
    expect(restored.allBills.every((item) => item.currencyCode == locale.currency), isTrue);
    expect(
    ...
    expect(restored.allRents.every((item) => item.currencyCode == locale.currency), isTrue);
    expect(restored.expenses.every((item) => item.currencyCode == locale.currency), isTrue);
  ...
  testWidgets('${locale.tag}: PDF generation uses the active language/report path', (
    tester,
  ) async {
    MizanClock.setNowForTesting(_now);
    ...
    expect(bytes.length, greaterThan(1000), reason: '${locale.tag}: PDF size');
    expect(String.fromCharCodes(bytes.take(4)), '%PDF', reason: locale.tag);
    expect(tester.takeException(), isNull, reason: '${locale.tag}: PDF render');
  });

  testWidgets('${locale.tag}: phone visits all primary screens without overflow', (
    tester,
  ) async {
    await _pumpApp(
      tester,
    ...
    final expectedDirection = const {'ar', 'fa', 'he', 'ur'}.contains(locale.tag)
        ? TextDirection.rtl
        : TextDirection.ltr;
    expect(
      Directionality.of(
  ...
  testWidgets('${locale.tag}: tablet visits all primary screens without overflow', (
    tester,
  ) async {
  ...
  testWidgets('${locale.tag}: all record dialogs render localized and overflow-free', (
    tester,
  ) async {
