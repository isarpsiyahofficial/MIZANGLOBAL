...
void main() {
  ...
  test('Indonesian source contains exactly 791 complete static values', () {
    ...
    expect(
      RegExp(r'[\u0400-\u052f\u0590-\u0dff\u2e80-\u9fff]').hasMatch(values),
      isFalse,
    );
  ...
  test('Indonesian uses stable natural financial terminology', () {
    ...
    expect(
      ...
      'Kewajiban pembayaran terlambat',
    );
    expect(
      MizanI18n.text('Yaklaşan ödeme yükü'),
      'Kewajiban pembayaran mendatang',
    );
  ...
  test(
    'Indonesian money numbers and Gregorian dates follow Indonesia conventions',
    () {
      MizanI18n.setProfile(languageTag: 'id', currencyCode: 'IDR');
  ...
  test(
    'Indonesian catalogs are complete natural and searchable offline',
    () async {
      MizanI18n.setProfile(languageTag: 'id', currencyCode: 'IDR');
      ...
      expect(
        catalog.languages.every((item) => item.nameFor('id').trim().isNotEmpty),
        isTrue,
      );
      expect(
        catalog.countries.every((item) => item.nameFor('id').trim().isNotEmpty),
        isTrue,
      );
      expect(
        catalog.currencies.every(
          (item) => item.nameFor('id').trim().isNotEmpty,
        ),
        isTrue,
      );
      expect(
        catalog.language('id').nameFor('id').toLowerCase(),
        contains('indonesia'),
      );
      expect(catalog.country('ID').nameFor('id'), 'Indonesia');
      expect(
        catalog.currency('IDR').nameFor('id').toLowerCase(),
        contains('rupiah'),
      );
      expect(
        catalog.countries
            .where((item) => item.matches('Indonesia'))
            .any((item) => item.code == 'ID'),
        isTrue,
      );
      expect(
        catalog.currencies
            .where((item) => item.matches('rupiah'))
            .any((item) => item.code == 'IDR'),
        isTrue,
      );
    },
  );
  ...
  test(
    'switching languages never leaks Indonesian into Turkish Arabic or English',
    () {
