...
void main() {
  const tags = <String>[
    'tr','en','es','pt-BR','pt-PT','fr','de','it','nl','pl','ro','el','ru','uk',
    'ar','fa','he','hi','bn','ur','id','ms','fil','vi','th','sw','zh','ja','ko',
  ];
  ...
  test('authoritative runtime exposes exactly 29 language options', () {
    ...
    for (final alias in const ['id-ID','in-ID','fil-PH','tl-PH','ko-KR','ja-JP','zh-CN','zh-Hans','vi-VN','th-TH','sw-TZ','sw-KE']) {
      expect(MizanI18n.isSupported(alias), isTrue, reason: alias);
    }
  });

  test('all 27 explicit translated static maps own the exact 791-key contract', () {
    ...
    for (final entry in maps.entries) {
      ...
      expect(entry.value.keys.toSet(), reference, reason: 'keys: ${entry.key}');
      expect(entry.value.values.every((value) => value.trim().isNotEmpty), isTrue, reason: 'empty: ${entry.key}');
      expect(entry.value.entries.where((item) => item.value.trim() == item.key.trim()).length, lessThan(120), reason: 'excessive source fallback: ${entry.key}');
    }
  });
  ...
  test('critical product surfaces survive repeated 29-language switching without previous-language retention', () {
    const source = ['Ana sayfa','Kayıtlar','Giderler','Raporlar','Ayarlar','Bildirim sistemi','PDF raporu','Kalan ödeme yükü'];
    ...
    for (final tag in [...tags, ...tags.reversed]) {
      ...
      if (tag != 'tr') {
        expect(current, isNot(source.join(' | ')), reason: 'raw Turkish fallback: $tag');
      }
      signatures[tag] = current;
    }
    for (final tag in const ['ko','ja','zh','vi','th','sw','id','ms','fil','ur','ar','he']) {
  ...
  test('CJK and Thai system-script boundaries are mutually isolated', () {
    ...
    expect(RegExp(r'[\u0e00-\u0e7f]').hasMatch(mizanVietnamese.values.join('\n')), isFalse);
    expect(RegExp(r'[\u0e00-\u0e7f]').hasMatch(mizanSwahili.values.join('\n')), isFalse);
  });

  test('closely related new Latin languages reject high-signal neighboring fallbacks', () {
    final samples = <String, String>{
      'id': const ['Ana sayfa','Kayıtlar','Giderler','Ayarlar','Bildirim sistemi'].map((k) => mizanIndonesian[k]!).join(' ').toLowerCase(),
      'ms': const ['Ana sayfa','Kayıtlar','Giderler','Ayarlar','Bildirim sistemi'].map((k) => mizanMalay[k]!).join(' ').toLowerCase(),
      'fil': const ['Ana sayfa','Kayıtlar','Giderler','Ayarlar','Bildirim sistemi'].map((k) => mizanFilipino[k]!).join(' ').toLowerCase(),
      'vi': const ['Ana sayfa','Kayıtlar','Giderler','Ayarlar','Bildirim sistemi'].map((k) => mizanVietnamese[k]!).join(' ').toLowerCase(),
      'sw': const ['Ana sayfa','Kayıtlar','Giderler','Ayarlar','Bildirim sistemi'].map((k) => mizanSwahili[k]!).join(' ').toLowerCase(),
    };
  ...
  test('user-authored mixed-script content remains visible under every language', () {
    const raw = 'Banka A.Ş. | قرض | اردو | हिन्दी | বাংলা | 한국어 | 日本語 | 中文 | ภาษาไทย | Việt Nam | M-Pesa';
    for (final tag in tags) {
      MizanI18n.setProfile(languageTag: tag, currencyCode: 'USD');
      final visible = MizanI18n.text(MizanI18n.user(raw));
      for (final marker in const ['Banka A.Ş.','한국어','日本語','中文','ภาษาไทย','Việt Nam','M-Pesa']) {
