...
void main() {
  test('language change reschedules system notifications without changing data', () async {
  ...
  test('region-only change does not rewrite the notification schedule', () async {
    final store = MemoryStore(comprehensiveState().copyWith(
      setupCompleted: true,
      appLanguageTag: 'en',
  ...
  test('default-currency-only change does not rewrite existing notification plan', () async {
    final store = MemoryStore(comprehensiveState().copyWith(
      setupCompleted: true,
      appLanguageTag: 'en',
      debtRegionCountryCode: 'US',
