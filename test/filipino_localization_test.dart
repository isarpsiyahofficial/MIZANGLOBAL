...
void main() {
  ...
  test(
    'Filipino source is complete and key-identical to the 791 product set',
    () {
  ...
  test('Filipino locale variants and Tagalog compatibility resolve safely', () {
    expect(MizanI18n.isSupported('fil'), isTrue);
    expect(MizanI18n.isSupported('fil-PH'), isTrue);
    expect(MizanI18n.isSupported('tl-PH'), isTrue);
    expect(MizanI18n.normalizeLanguageTag('FIL_ph'), 'fil');
    expect(MizanI18n.normalizeLanguageTag('tl_PH'), 'fil');
  });
  test('Filipino finance and report terms are distinct and natural', () {
    MizanI18n.setProfile(languageTag: 'fil-PH', currencyCode: 'PHP');
    expect(MizanI18n.text('Ana sayfa'), 'Simula');
    expect(MizanI18n.text('Kayıtlar'), 'Mga tala');
    expect(MizanI18n.text('Giderler'), 'Mga gastusin');
    expect(MizanI18n.text('Raporlar'), 'Mga ulat');
    expect(MizanI18n.text('Ayarlar'), 'Mga setting');
    expect(MizanI18n.text('Banka borcu'), 'Utang sa bangko');
    expect(
    ...
    expect(
      ...
      'Overdue na obligasyon sa pagbabayad',
    );
    expect(MizanI18n.destructiveConfirmation, 'KINUKUMPIRMA KO');
    expect(MizanI18n.text('3 gün kaldı'), '3 araw na lang');
    expect(
      MizanI18n.text('Ödeme 5 gün gecikti.'),
      'Overdue ng 5 araw ang bayad.',
    );
  });
  test(
    'Philippine peso, numbers and Gregorian dates follow PH conventions',
    () {
      ...
      expect(shortDate(DateTime(2026, 8, 7)), 'Ago 7, 2026');
      expect(monthLabel(DateTime(2026, 8)), 'Agosto 2026');
      expect(parseMoney('₱1,234,567.50'), 1234567.5);
      expect(parseMoney('PHP 1,234,567.50'), 1234567.5);
      MizanI18n.setProfile(languageTag: 'fil', currencyCode: 'USD');
      expect(money(1234.5), 'USD\u00A01,234.50');
    },
  );
  test(
    'Filipino offline catalogs contain every global selector item',
    () async {
      MizanI18n.setProfile(languageTag: 'fil', currencyCode: 'PHP');
      ...
      expect(catalog.language('fil').nameFor('fil'), 'Filipino');
      expect(catalog.country('PH').nameFor('fil'), 'Pilipinas');
      expect(catalog.currency('PHP').nameFor('fil'), 'Piso ng Pilipinas');
      expect(
        catalog.countries
            .where((e) => e.matches('Pilip'))
            .any((e) => e.code == 'PH'),
        isTrue,
      );
      expect(
        catalog.currencies
            .where((e) => e.matches('Piso'))
            .any((e) => e.code == 'PHP'),
        isTrue,
      );
    },
