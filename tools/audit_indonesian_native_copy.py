...
def parse_maps(directory: Path) -> dict[str, str]:
    ...
    for path in sorted(directory.glob('mizan_*_*.dart')):
        for line_no, line in enumerate(path.read_text(encoding='utf-8').splitlines(), 1):
            ...
            if key in result:
                raise AssertionError(f'duplicate key {key!r} in {path}:{line_no}')
            result[key] = value
    return result
...
def main() -> int:
    ...
    if missing:
        errors.append(f'missing Indonesian keys ({len(missing)}): {missing[:12]}')
    if extra:
        errors.append(f'extra Indonesian keys ({len(extra)}): {extra[:12]}')
    ...
    if TURKISH_MARKERS.search(re.sub(r"RegExp\([^\n]+", '', dynamic)):
        errors.append('Turkish fallback detected in Indonesian dynamic output copy')
    for required in ('translateIndonesianReviewedDynamic', 'Jatuh tempo hari ini', 'SAYA SETUJU'):
        haystack = dynamic + joined
