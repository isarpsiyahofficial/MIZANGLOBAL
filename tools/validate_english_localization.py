...
for path in LIB.rglob("*.dart"):
    ...
    if path == I18N or rel == "lib/global/global_catalog.dart":
        continue
...
if "static const supportedLanguageTags = <String>{'tr', 'en'};" not in text:
    failures.append("only Turkish and English must be enabled")
main_source = (LIB / "main.dart").read_text(encoding="utf-8")
if "supportedLocales: const [Locale('tr'), Locale('en')]" not in main_source:
    failures.append("MaterialApp must expose exactly Turkish and English")
...
pdf_source = (LIB / "services" / "pdf_report_service.dart").read_text(encoding="utf-8")
if pdf_source.count("TextPainter(") != 1 or "final localizedText = MizanI18n.text(" not in pdf_source:
    failures.append("all rasterized PDF text must pass through the centralized localizer")
reminder_source = (LIB / "services" / "reminder_engine.dart").read_text(encoding="utf-8")
if "title: MizanI18n.text(" not in reminder_source or "message: MizanI18n.text(" not in reminder_source:
