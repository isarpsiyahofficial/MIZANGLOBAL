...
def main():
    ...
    assert not re.search(r'[ğışçöüİ]',joined),'Turkish system copy leaked into Filipino'
    ...
    for marker in ("currencyCode=='PHP'",'_filipinoMonths','₱'): assert marker in formatters,marker
    assert "Locale('fil','PH')" in main_dart
