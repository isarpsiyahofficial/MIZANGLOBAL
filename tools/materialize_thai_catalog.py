...
lang_alias = {'pt-BR':'pt', 'pt-PT':'pt', 'fil':'fil', 'zh':'zh', 'he':'he'}
langs = {code: str(locale.languages.get(lang_alias.get(code, code), code)) for code in language_codes}
langs.update({'pt-BR':'โปรตุเกส (บราซิล)','pt-PT':'โปรตุเกส (โปรตุเกส)','fil':'ฟิลิปปินส์','zh':'จีน','th':'ไทย'})
...
currencies.update({'XCG':'กิลเดอร์แคริบเบียน','ZWG':'ซิมบับเวโกลด์'})
assert len(langs) == 29 and len(countries) == 161 and len(currencies) == 154
assert all(v and v != k for k, v in langs.items())
assert all(v for v in countries.values()) and all(v for v in currencies.values())
...
out = '// Reviewed offline Thai catalog names used by global selectors.\n' + render('thaiLanguageNames', langs) + render('thaiCountryNames', countries) + render('thaiCurrencyNames', currencies)
path = ROOT / 'lib' / 'l10n' / 'th' / 'mizan_th_catalog.dart'
