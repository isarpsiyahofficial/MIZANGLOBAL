// REVIEWED HINDI LOCALIZATION — NATURAL INDIA-ORIENTED PRODUCT COPY.
const Map<String, String> mizanHindiDashboard = <String, String>{
  'Kayıt doğrulaması başarısız oldu.': 'रिकॉर्ड का सत्यापन विफल रहा।',
  'Detayı gör': 'विवरण देखें',
  'Not ekle': 'नोट जोड़ें',
  'Bu kayda ait not bulunmuyor. Notlar ödeme açıklamalarından ayrı tutulur.':
      'इस रिकॉर्ड के लिए कोई नोट्स नहीं हैं. नोट भुगतान विवरण से अलग संग्रहीत किए जाते हैं।',
  'Notu sil': 'नोट हटाएँ',
  'Notları daralt': 'नोट समेटें',
  'Not boş bırakılamaz.': 'नोट आवश्यक है।',
  'Yalnızca bu not silinecek. Devam edilsin mi?':
      'केवल यही नोट हटेगा। जारी रखें?',
  'Borç, ödeme ve giderlerin sade özeti. Detay görmek için kartlara dokunabilirsin.':
      'कर्ज़, भुगतान और खर्चों का साफ़ सारांश। विवरण देखने के लिए कार्ड पर टैप करें।',
  'Bu Ayın Ödeme Durumu': 'इस महीने की भुगतान स्थिति',
  'Gecikmiş ödemeler': 'बकाया भुगतान',
  'Bugünkü normal gider': 'आज के सामान्य खर्च',
  'Bu ay normal gider': 'इस महीने के सामान्य खर्च',
  'Bugünkü ödemelere yapılan gider': 'आज किए गए भुगतान',
  'Bu ay ödemelere yapılan gider': 'इस महीने किए गए भुगतान',
  'Bugünkü toplam gider': 'आज का कुल खर्च',
  'Normal giderler ile banka, şahıs, fatura, abonelik, kira ve taksit ödemelerinin toplamıdır.':
      'यह सामान्य खर्चों और बैंक कर्ज़, व्यक्तिगत कर्ज़, बिल, सदस्यता, किराया तथा किस्त भुगतानों का कुल है।',
  'Bu ay toplam gider': 'इस महीने का कुल खर्च',
  'Bu ayın normal giderleri ile kaydedilmiş tüm ödeme giderlerinin toplamıdır.':
      'इसमें इस महीने के नियमित खर्च और सभी रिकॉर्ड किए गए भुगतान शामिल हैं।',
  'Kritik ödemeler': 'ज़रूरी भुगतान',
  'Gecikmiş veya yedi gün içinde vadesi gelen kayıtlar. Ayrıntı için satıra dokun.':
      'ऐसे रिकॉर्ड जो बकाया हैं या सात दिनों के भीतर देय हैं। विवरण के लिए किसी पंक्ति पर टैप करें।',
  'Kritik ödeme yok': 'कोई ज़रूरी भुगतान नहीं',
  'Gecikmiş veya önümüzdeki yedi gün içinde vadesi gelen kayıt bulunmuyor.':
      'अगले सात दिनों के भीतर कोई बकाया रिकॉर्ड या रिकॉर्ड देय नहीं है।',
  'Uygulama boş ve kullanıma hazır': 'ऐप खाली है और उपयोग के लिए तैयार है',
  'Örnek ödeme veya borç oluşturulmadı. Kayıtlar bölümünden ilk kişiyi ekleyerek başlayabilirsin.':
      'कोई नमूना भुगतान या कर्ज़ नहीं बनाया गया। रिकॉर्ड में पहला व्यक्ति जोड़कर शुरुआत करें।',
  'Gelir bilgileri': 'आय विवरण',
  'Gelir ekle': 'आय जोड़ें',
  'Gelir kaydı opsiyoneldir. Borç ödemeleri ve giderler gelirden ayrı tutulur; net sonuç raporda hesaplanır.':
      'आय जोड़ना वैकल्पिक है। कर्ज़ के भुगतान और खर्च आय से अलग रखे जाते हैं; शुद्ध परिणाम रिपोर्ट में निकाला जाता है।',
  'Gelir bilgisi belirtilmemiş': 'आय की कोई जानकारी नहीं दी गई',
  'Tek seferlik, günlük, haftalık veya aylık gelir ekleyebilirsin.':
      'आप एकमुश्त, दैनिक, साप्ताहिक या मासिक आय जोड़ सकते हैं।',
  'Gelir yattı': 'आय मिल गई',
  'Son alınma işaretini geri al': 'आय मिलने का अंतिम निशान हटाएँ',
  'Arşivden çıkar': 'आर्काइव से वापस लाएँ',
  'Arşivle': 'आर्काइव करें',
  'Geliri düzenle': 'आय संपादित करें',
  'Gelir türü / adı': 'आय का प्रकार/नाम',
  'Maaş, ek iş, kira geliri…': 'वेतन, फ्रीलांस काम, किराये की आय…',
  'Gelir türü boş bırakılamaz.': 'आय प्रकार आवश्यक है।',
  'Gelir tutarı sıfırdan büyük olmalıdır.': 'आय राशि शून्य से अधिक होनी चाहिए।',
  'Gelir sıklığı': 'आय मिलने की आवृत्ति',
  'Yatış gününü takip et': 'आय मिलने की तारीख ट्रैक करें',
  'Opsiyoneldir. Planlanan gün ile gerçek alınma tarihi ayrı tutulur.':
      'वैकल्पिक। तय तारीख और वास्तव में आय मिलने की तारीख अलग-अलग रखी जाती हैं।',
  'Haftanın hangi günü yatıyor?': 'आय सप्ताह के किस दिन मिलती है?',
  'Her ayın kaçında yatıyor?': 'आय हर महीने किस तारीख को मिलती है?',
  'Ay daha kısaysa o ayın son geçerli günü kullanılır.':
      'यदि महीना छोटा है, तो उसके अंतिम वैध दिन का उपयोग किया जाता है।',
  'Gelir başlangıç tarihini seçin': 'आय प्रारंभ तिथि का चयन करें',
  'Gelir notu (opsiyonel)': 'आय नोट (वैकल्पिक)',
  'Salı': 'मंगलवार',
  'Çarşamba': 'बुधवार',
  'Perşembe': 'गुरुवार',
  'Pazartesi': 'सोमवार',
  'Cuma': 'शुक्रवार',
  'Cumartesi': 'शनिवार',
  'Pazar': 'रविवार',
  'Gün': 'दिन',
  'Başlangıç': 'आरंभ',
  'Arşivde': 'आर्काइव में',
  'Gelirin gerçekten alındığı tarihi seçin':
      'वह तारीख चुनें जब आय वास्तव में प्राप्त हुई थी',
  'Kalan toplam borç detayı': 'कुल बाकी कर्ज़ का विवरण',
  'Her bölümün toplamı ayrı hesaplanır. Satıra dokunarak yalnız ilgili kayıtları görebilirsin.':
      'प्रत्येक अनुभाग की गणना अलग से की जाती है। केवल संबंधित रिकॉर्ड देखने के लिए किसी पंक्ति पर टैप करें।',
  'Ödeme Durumu': 'भुगतान स्थिति',
  'Açık planlanan kayıtlar ile bu ay gerçekten yapılan ödemeler ayrı gösterilir.':
      'बकाया निर्धारित रिकॉर्ड और इस महीने वास्तव में किए गए भुगतान अलग-अलग दिखाए गए हैं।',
  'Açık planlanan ödemeler': 'बाकी निर्धारित भुगतान',
  'Açık plan kalmadı': 'कोई निर्धारित भुगतान बाकी नहीं',
  'Bu aya ait açık veya eksik ödeme bulunmuyor.':
      'इस माह का कोई बकाया या अधूरा भुगतान नहीं है।',
  'Bu ay yapılan ödemeler': 'इस महीने किए गए भुगतान',
  'Yapılan ödeme yok': 'कोई भुगतान नहीं किया गया',
  'Bu ay ödeme geçmişine kaydedilmiş işlem bulunmuyor.':
      'इस महीने भुगतान इतिहास में कोई लेनदेन दर्ज नहीं किया गया है।',
  'Kayıt bulunmuyor': 'कोई रिकॉर्ड नहीं मिला',
  'Bu başlığa ait açık ödeme kaydı yok.':
      'इस शीर्षक के अंतर्गत कोई बकाया भुगतान रिकॉर्ड नहीं है।',
  'Gelir özeti': 'आय सारांश',
  'Yönet': 'प्रबंधित करें',
  'Bu ay gelir': 'इस माह आय',
  'Ödemeler sonrası kalan': 'भुगतान के बाद शेष',
  'Ödeme ve gider sonrası net': 'भुगतान और खर्च के बाद शुद्ध राशि',
  'Ödemeler': 'भुगतान',
  'Bütün harcamalar': 'सभी खर्च',
  'Bu ay': 'इस महीने',
  'Son 30 gün': 'पिछले 30 दिन',
  'Son 90 gün': 'पिछले 90 दिन',
  'Tarih aralığı': 'तारीख सीमा',
  'Tümü': 'सभी',
  'Harcamalar gün gün gruplanır; arama ve günlük toplam sıralaması uzun yıllarda da kontrollü çalışır.':
      'खर्च को दिन के अनुसार समूहीकृत किया जाता है; खोज और दैनिक-कुल सॉर्टिंग कई वर्षों के डेटा में भी कुशल रहती है।',
  'Bugün': 'आज',
  'Filtreleme ve arama': 'फ़िल्टर और खोज',
  'Tarih, gün adı, gider, kategori veya not yazabilirsiniz. Türkçe karakterler ve bitişik ifadeler eşleşir.':
      'तारीख, दिन, खर्च, श्रेणी या नोट से खोजें। उच्चारण चिह्न और जुड़े हुए शब्द भी मिलान किए जाते हैं।',
  'Gider veya tarih ara': 'खर्च या तारीखें खोजें',
  'Araç, yoğurt, 23.07.2026, Perşembe…': 'कार, दही, 23/07/2026, गुरुवार…',
  'Günleri sırala': 'दिनों को क्रम में लगाएँ',
  'Tüm kategoriler': 'सभी श्रेणियां',
  'Kategori ekle': 'श्रेणी जोड़ें',
  'Önce kategori ekleyin': 'पहले एक श्रेणी जोड़ें',
  'Market, ulaşım veya kullanıcıya özel başka bir kategori ekledikten sonra gider kaydı oluşturabilirsiniz.':
      'किराना, यात्रा या अपनी पसंद की कोई श्रेणी जोड़ने के बाद खर्च रिकॉर्ड बनाया जा सकता है।',
  'Eşleşen gider bulunamadı': 'मेल खाता कोई खर्च नहीं मिला',
  'Seçili kategori, dönem ve arama ifadesine uyan kayıt yok.':
      'कोई भी रिकॉर्ड चयनित श्रेणी, अवधि और खोज शब्द से मेल नहीं खाता।',
  'Daha fazla gün göster': 'और दिन दिखाएँ',
  'Bütün harcamalar görünümünde günlük harcamalar ve ödemeler ayrı başlıklar altında tutulur; yalnız toplamları birlikte hesaplanır.':
      'सभी खर्चों में, दैनिक खर्च और भुगतान अलग-अलग वर्गों में रहते हैं; केवल उनका योग संयुक्त है।',
  'Tarih aralığı seçin': 'दिनांक सीमा चुनें',
  'Gider kategorileri': 'खर्च श्रेणियाँ',
  'Kategori silinirse yalnız o kategoriye bağlı giderler açık onayla silinir.':
      'श्रेणी हटाने पर स्पष्ट पुष्टि के बाद केवल उसी श्रेणी से जुड़े खर्च हटेंगे।',
  'Kategoriyi düzenle': 'श्रेणी संपादित करें',
  'Kategori adı boş bırakılamaz.': 'श्रेणी का नाम आवश्यक है।',
  'Kategoriyi sil': 'श्रेणी हटाएँ',
  'ONAYLIYORUM yazın': '“मैं सहमत हूँ” लिखें',
  'Tam olarak ONAYLIYORUM yazılmalı.':
      'ठीक-ठीक “मैं सहमत हूँ” लिखना ज़रूरी है।',
  'Gideri düzenle': 'खर्च संपादित करें',
  'Gider adı boş bırakılamaz.': 'खर्च नाम आवश्यक है।',
  'Adet / miktar': 'संख्या / मात्रा',
  'Birim fiyat negatif olamaz.': 'इकाई मूल्य ऋणात्मक नहीं हो सकता।',
  'Gideri sil': 'खर्च हटाएँ',
  'Banka / kredi': 'बैंक / क्रेडिट',
  'Kişisel / kurumsal': 'व्यक्तिगत/व्यावसायिक',
  'Ödeme bulunamadı': 'कोई भुगतान नहीं मिला',
  'Seçili filtrede kaydedilmiş ödeme yok.':
      'चयनित फ़िल्टर के लिए कोई भुगतान रिकॉर्ड नहीं किया गया है।',
  'Daha fazla ödeme günü göster': 'और भुगतान वाले दिन दिखाएँ',
  'Kategori bulunamadı': 'श्रेणी नहीं मिली',
  'Bu günden daha fazla göster': 'इस दिन के और रिकॉर्ड दिखाएँ',
  'Gider işlemleri': 'खर्च से जुड़ी कार्रवाइयाँ',
  'Önce kişiyi seç, ardından kayıt türünü aç. Her bölüm birbirinden bağımsız tutulur.':
      'पहले एक व्यक्ति का चयन करें, फिर एक रिकॉर्ड प्रकार खोलें। प्रत्येक अनुभाग को स्वतंत्र रखा गया है।',
  'Kişi ekle': 'व्यक्ति जोड़ें',
  'Henüz kişi yok': 'अभी कोई व्यक्ति नहीं है',
  'Kayıtların birbirine karışmaması için önce ödeme ve gider kayıtlarının sahibi olacak kişiyi ekleyin.':
      'पहले उस व्यक्ति को जोड़ें जिसके पास भुगतान और खर्च रिकॉर्ड होंगे, ताकि रिकॉर्ड अलग रहें।',
  'İlk kişiyi ekle': 'पहला व्यक्ति जोड़ें',
  'Kişisel ve Kurumsal Borçlar': 'व्यक्तिगत और व्यावसायिक कर्ज़',
  'Kişi, şirket/kurum, çek, senet, esnaf/işletme, aile/yakın ve diğer alacaklılar':
      'लोग, कंपनियां/संस्थाएं, चेक, वचन पत्र, व्यापारी/व्यवसाय, परिवार/रिश्तेदार, और अन्य लेनदार',
  'Kişisel / kurumsal borç ekle': 'व्यक्तिगत / व्यावसायिक कर्ज़ जोड़ें',
  'Banka dışı borç kaydı bulunmuyor.':
      'बैंक के बाहर का कोई कर्ज़ रिकॉर्ड नहीं है।',
  'Elektrik, su, telefon, internet, doğalgaz ve özel faturalar':
      'बिजली, पानी, फोन, इंटरनेट, प्राकृतिक गैस और कस्टम बिल',
  'Fatura ekle': 'बिल जोड़ें',
  'Fatura kaydı bulunmuyor.': 'कोई बिल रिकॉर्ड नहीं मिला।',
  'Belirli aralıklarla tekrarlayan dijital hizmet, üyelik, sigorta, eğitim ve bakım ödemeleri':
      'तय अंतराल पर दोहराए जाने वाले डिजिटल सेवा, सदस्यता, बीमा, शिक्षा और रखरखाव भुगतान',
  'Abonelik ekle': 'सदस्यता जोड़ें',
  'Abonelik kaydı bulunmuyor.': 'कोई सदस्यता रिकॉर्ड नहीं मिला।',
  'Kira ve Taksitler': 'किराया और किस्तें',
  'Ev/iş yeri kirası, ürün taksiti ve düzenli ödeme planları':
      'घर या कार्यस्थल का किराया, उत्पाद की किस्त और नियमित भुगतान योजनाएँ',
  'Kira / taksit ekle': 'किराया/किस्त जोड़ें',
  'Kira veya taksit kaydı bulunmuyor.':
      'कोई किराया या किस्त संबंधी रिकॉर्ड नहीं मिला।',
};
