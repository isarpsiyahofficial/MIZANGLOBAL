const Map<String, String> mizanHindiValidation = <String, String>{
  'Her değişiklik cihazda anında kaydedilir; sağlam kayıt doğrulanmadan üzerine yazılmaz.':
      'प्रत्येक परिवर्तन तुरंत डिवाइस में सहेजा जाता है; नए सेव के सत्यापित होने से पहले वैध डेटा को कभी भी अधिलेखित नहीं किया जाता है।',
  'Kişiler, borçlar, faturalar, abonelikler, ödemeler, notlar, gelirler ve giderler her işlemden sonra cihazdaki dosyaya yazılır.':
      'हर कार्रवाई के बाद व्यक्ति, कर्ज़, बिल, सदस्यता, भुगतान, नोट, आय और खर्च डिवाइस की फ़ाइल में सहेजे जाते हैं।',
  'Yeni kayıt doğrulandıktan sonra ana dosya değiştirilir; son sağlam kopya ayrıca korunur.':
      'नया डेटा सत्यापित होने के बाद ही मुख्य फ़ाइल को बदला जाता है; अंतिम वैध प्रति अलग से सुरक्षित रखी जाती है।',
  'Yedek içe aktarılırken mevcut kayıtlar silinmez. Ortak kayıtlar atlanır, yalnız yeni kayıtlar ve eksik ilişkiler eklenir.':
      'बैकअप आयात करने से मौजूदा रिकॉर्ड नहीं हटते। मिलान रिकॉर्ड छोड़ दिए गए हैं; केवल नए रिकॉर्ड और लुप्त रिश्ते जोड़े जाते हैं।',
  'Kişi, banka, borç, ödeme, not, kategori, gider, gelir ve bildirim saatleri kendi kimlik ve bağlantılarıyla aktarılır. Aynı kayıt ikinci kez yazılmaz.':
      'व्यक्ति, बैंक, कर्ज़, भुगतान, नोट, श्रेणी, खर्च, आय और सूचना समय अपनी मूल आईडी और संबंधों के साथ स्थानांतरित होते हैं। एक ही रिकॉर्ड दूसरी बार नहीं लिखा जाता।',
  'Uygulama dili seçilmelidir.': 'ऐप की भाषा चुनना ज़रूरी है।',
  'Ülke kodu geçersiz.': 'अमान्य देश कोड।',
  'Para birimi kodu geçersiz.': 'अमान्य मुद्रा कोड।',
  'Tamamlanmış profilde uygulama dili eksik.':
      'पूर्ण प्रोफ़ाइल में ऐप भाषा गुम है।',
  'Tamamlanmış profilde ülke kodu geçersiz.':
      'पूर्ण प्रोफ़ाइल में एक अमान्य देश कोड है।',
  'Tamamlanmış profilde para birimi kodu geçersiz.':
      'पूर्ण प्रोफ़ाइल में एक अमान्य मुद्रा कोड है।',
  'Global katalog henüz yüklenmedi.': 'वैश्विक कैटलॉग अभी लोड नहीं हुआ है।',
  'Global katalog sayıları doğrulanamadı.':
      'वैश्विक कैटलॉग की संख्या सत्यापित नहीं हो सकी।',
  'Bildirim izni veya zamanlama servisi açılamadı:':
      'सूचना अनुमति या शेड्यूलिंग सेवा नहीं खोली जा सकी:',
  'Yerel kayıt alanı güvenli biçimde açılamadı. Mevcut dosyaları korumak için yeni veri yazımı durduruldu.':
      'स्थानीय संग्रहण सुरक्षित रूप से नहीं खुल सका। मौजूदा फ़ाइलों को बचाने के लिए नया डेटा लिखना रोक दिया गया है।',
  'Bildirim izni kapalı. Android izni açıldığında MİZAN otomatik olarak yeniden senkronize eder.':
      'सूचना अनुमति बंद है. Android अनुमति सक्षम होने के बाद MİZAN स्वचालित रूप से पुन: सिंक्रनाइज़ हो जाएगा।',
  'Dakik bildirim izni kapalı. Android izni açıldığında MİZAN otomatik olarak yeniden senkronize eder.':
      'सटीक अलार्म अनुमति बंद है. Android अनुमति सक्षम होने के बाद MİZAN स्वचालित रूप से पुन: सिंक्रनाइज़ हो जाएगा।',
  'Kayıt yapıldı ancak bildirimler otomatik senkronize edilemedi:':
      'रिकॉर्ड सहेजा गया था, लेकिन सूचनाएं स्वचालित रूप से सिंक्रनाइज़ नहीं की जा सकीं:',
  'Kişi adı': 'व्यक्ति का नाम',
  'Banka adı': 'बैंक का नाम',
  'Toplam borç': 'कुल कर्ज़',
  'Aylık tutar': 'मासिक राशि',
  'Gecikme günü': 'देरी के दिन',
  'Limit': 'सीमा',
  'Kullanılan limit': 'इस्तेमाल की गई सीमा',
  'Açıklama': 'विवरण',
  'Düzenli ödeme tutarı': 'नियमित भुगतान राशि',
  'Borç başlığı': 'कर्ज़ का शीर्षक',
  'Alacaklı adı': 'लेनदार का नाम',
  'Çek numarası': 'चेक नंबर',
  'Düzenleyen': 'जारीकर्ता',
  'Banka bilgisi': 'बैंक संबंधी जानकारी',
  'Senet numarası': 'वचन पत्र क्रमांक',
  'Ödeme planı tutarı': 'भुगतान योजना राशि',
  'Abonelik tutarı': 'सदस्यता राशि',
  'Abonelik türü': 'सदस्यता प्रकार',
  'Abonelik başlığı': 'सदस्यता शीर्षक',
  'Sağlayıcı adı': 'प्रदाता का नाम',
  'Abone numarası': 'ग्राहक संख्या',
  'Sözleşme numarası': 'करार संख्या',
  'Fatura tutarı': 'बिल राशि',
  'Dönem fatura tutarı': 'बिलिंग-अवधि राशि',
  'Kurum adı': 'संस्था का नाम',
  'Kira/taksit tutarı': 'किराया/किस्त राशि',
  'Kira/taksit başlığı': 'किराया/किस्त का शीर्षक',
  'Alıcı adı': 'प्राप्तकर्ता का नाम',
  'IBAN': 'IBAN',
  'Adet': 'मात्रा',
  'Birim fiyat': 'यूनिट मूल्य',
  'Gider adı': 'खर्च नाम',
  'Gider notu': 'खर्च नोट',
  'Ödeme tutarı': 'भुगतान राशि',
  'Ödeme notu': 'भुगतान नोट',
  'Ödeme yöntemi': 'भुगतान विधि',
  'Not': 'नोट',
  'Notlar': 'नोट्स',
  'Kategori adı': 'श्रेणी नाम',
  'Gelir tutarı': 'आय राशि',
  'Gelir türü': 'आय का प्रकार',
  'Gelir notu': 'आय नोट',
  'Hatırlatma adı': 'रिमाइंडर नाम',
  'Bildirim mesajı': 'सूचना संदेश',
  'Geçici': 'अस्थायी',
  'Ödeme hatırlatması': 'भुगतान रिमाइंडर',
  'Yaklaşan ve gecikmiş ödemelerini kontrol et.':
      'अपने आगामी और बकाया भुगतानों की समीक्षा करें।',
  'En fazla 10 ödeme bildirimi eklenebilir.':
      'आप अधिकतम 10 भुगतान सूचनाएं जोड़ सकते हैं।',
  'Ödeme bildirim saati bulunamadı.': 'भुगतान सूचना का समय नहीं मिला।',
  'Bildirim saati geçersiz.': 'अमान्य सूचना समय।',
  'En az bir ödeme bildirim saati bulunmalıdır.':
      'कम से कम एक भुगतान सूचना समय आवश्यक है।',
  'Gelir kaydı bulunamadı.': 'आय का रिकॉर्ड नहीं मिला।',
  'Haftalık gelir için geçerli bir gün seçilmelidir.':
      'साप्ताहिक आय के लिए एक वैध कार्यदिवस चुनें।',
  'Aylık gelir günü 1 ile 31 arasında olmalıdır.':
      'मासिक आय का दिन 1 से 31 के बीच होना चाहिए।',
  'Yatış günü takibi yalnız haftalık ve aylık gelirlerde kullanılabilir.':
      'आय मिलने की तारीख केवल साप्ताहिक और मासिक आय के लिए ट्रैक की जा सकती है।',
  'Bu gelir için yatış günü takibi açık değil.':
      'इस आय के लिए मिलने की तारीख ट्रैक करना चालू नहीं है।',
  'Bu gelir dönemi daha önce alındı olarak işaretlenmiş.':
      'इस आय अवधि को पहले ही “मिल गई” के रूप में दर्ज किया जा चुका है।',
  'Geri alınacak gelir işareti yok.':
      'पूर्ववत करने के लिए कोई आय प्राप्ति स्थिति नहीं है।',
  'Bildirim ayarı bulunamadı.': 'सूचना सेटिंग नहीं मिली।',
  'Ödeme kalan borçtan büyük olamaz.':
      'भुगतान बाकी कर्ज़ से अधिक नहीं हो सकता।',
  'Borç kaydı bulunamadı.': 'कर्ज़ का रिकॉर्ड नहीं मिला।',
  'Ödeme kalan fatura tutarından büyük olamaz.':
      'भुगतान शेष बिल राशि से अधिक नहीं हो सकता।',
  'Ödeme aboneliğin bu dönem kalan tutarından büyük olamaz.':
      'भुगतान इस अवधि के लिए सदस्यता की शेष राशि से अधिक नहीं हो सकता।',
  'Ödeme kalan kira/taksit tutarından büyük olamaz.':
      'भुगतान शेष किराया/किस्त राशि से अधिक नहीं हो सकता।',
  'Ödeme kaydı bulunamadı.': 'भुगतान रिकॉर्ड नहीं मिला।',
  'Güncellenen ödeme toplam tutarı aşamaz.':
      'अद्यतन भुगतान कुल राशि से अधिक नहीं हो सकता।',
  'Toplam borç, daha önce ödenen tutardan düşük olamaz.':
      'कुल कर्ज़ पहले से चुकाई गई राशि से कम नहीं हो सकता।',
  'Fatura tutarı, daha önce ödenen tutardan düşük olamaz.':
      'बिल की राशि पहले से भुगतान की गई राशि से कम नहीं हो सकती।',
  'Kira/taksit tutarı, daha önce ödenen tutardan düşük olamaz.':
      'किराया/किस्त राशि पहले से भुगतान की गई राशि से कम नहीं हो सकती।',
  'Her ayın belirli günü seçildiğinde aylık tutar girilmelidir.':
      'जब प्रत्येक माह का एक विशिष्ट दिन चुना जाता है तो मासिक राशि की आवश्यकता होती है।',
  'Gecikmiş ay seçimi yalnız aylık ödeme gününde kullanılabilir.':
      'बकाया माह का चयन केवल मासिक भुगतान दिवस के साथ ही उपलब्ध है।',
  'Gecikmiş ayın ödeme tarihi henüz gelmemiş olamaz.':
      'चयनित बकाया माह की देय तिथि भविष्य में नहीं हो सकती।',
  'Kullanılan limit toplam limiti aşamaz.':
      'प्रयुक्त क्रेडिट कुल सीमा से अधिक नहीं हो सकता।',
  'Son ödeme tarihi borç tarihinden önce olamaz.':
      'अंतिम भुगतान तारीख कर्ज़ बनने की तारीख से पहले नहीं हो सकती।',
  'Taksitli borçta ödeme tutarı girilmelidir.':
      'किस्त वाले कर्ज़ के लिए भुगतान राशि दर्ज करें।',
  'Özel ödeme aralığı gün olarak girilmelidir.':
      'दिनों में कस्टम भुगतान अंतराल दर्ज करें।',
  'Çek numarası boş bırakılamaz.': 'चेक नंबर आवश्यक है।',
  'Senet numarası boş bırakılamaz.': 'वचन पत्र संख्या आवश्यक है।',
  'Abonelik ödeme sıklığı tek ödeme olamaz.':
      'कोई सदस्यता एकमुश्त भुगतान आवृत्ति का उपयोग नहीं कर सकती।',
  'Aylık fatura günü 1 ile 31 arasında olmalıdır.':
      'मासिक बिल का दिन 1 से 31 के बीच होना चाहिए।',
  'Ödeme günü 1 ile 31 arasında olmalı.':
      'भुगतान का दिन 1 से 31 के बीच होना चाहिए।',
  'Ürün taksitinde toplam taksit sayısı gereklidir.':
      'किसी उत्पाद किस्त के लिए किस्तों की कुल संख्या आवश्यक है।',
  'Sözleşme bitişi başlangıçtan önce olamaz.':
      'अनुबंध की समाप्ति तिथि प्रारंभ तिथि से पहले नहीं हो सकती।',
  'Bir borç kaydında ödeme toplamı borcu aşıyor.':
      'कर्ज़ रिकॉर्ड के भुगतान कुल कर्ज़ से अधिक हैं।',
  'Bir kişisel borçta ödeme toplamı borcu aşıyor.':
      'व्यक्तिगत कर्ज़ के भुगतान कुल कर्ज़ से अधिक हैं।',
  'Bir fatura kaydında ödeme toplamı tutarı aşıyor.':
      'बिल पर भुगतान बिल राशि से अधिक है।',
  'Aylık fatura ödeme günü geçersiz.': 'अमान्य मासिक बिल भुगतान दिवस।',
  'Dönemsel fatura tutarı sıfırdan büyük olmalıdır.':
      'बिलिंग अवधि की राशि शून्य से अधिक होनी चाहिए।',
  'Bir kira kaydında ödeme toplamı tutarı aşıyor.':
      'किराये के रिकॉर्ड पर भुगतान देय राशि से अधिक है।',
  'Bir gider kaydı bulunmayan kategoriye bağlı.':
      'एक खर्च ऐसे श्रेणी से जुड़ा है जो मौजूद नहीं है।',
  'Kişi bulunamadı.': 'व्यक्ति नहीं मिला।',
  'Banka kaydı bulunamadı.': 'बैंक रिकॉर्ड नहीं मिला।',
  'Kişisel/kurumsal borç bulunamadı.':
      'व्यक्तिगत / व्यावसायिक कर्ज़ नहीं मिला।',
  'Abonelik kaydı bulunamadı.': 'सदस्यता रिकॉर्ड नहीं मिला।',
  'Fatura kaydı bulunamadı.': 'बिल रिकॉर्ड नहीं मिला।',
  'Kira/taksit kaydı bulunamadı.': 'किराया/किस्त का रिकॉर्ड नहीं मिला।',
  'Gider kategorisi bulunamadı.': 'खर्च की श्रेणी नहीं मिली।',
  'Gider kaydı bulunamadı.': 'खर्च का रिकॉर्ड नहीं मिला।',
  'Bu kişide aynı banka adı zaten var.':
      'इस व्यक्ति के पास पहले से ही इसी नाम से एक बैंक है।',
  'Bu kategori adı zaten kullanılıyor.':
      'यह श्रेणी नाम पहले से ही उपयोग में है।',
  'Banka borcu kaydı bulunamadı.': 'बैंक के कर्ज़ का रिकॉर्ड नहीं मिला।',
  'Toplam taksit pozitif olmalı.':
      'किस्तों की कुल संख्या सकारात्मक होनी चाहिए।',
  'Taksit ilerlemesi negatif olamaz.':
      'किस्त की प्रगति नकारात्मक नहीं हो सकती।',
  'Taksit ilerlemesi toplam taksiti aşamaz.':
      'किस्त की प्रगति किस्तों की कुल संख्या से अधिक नहीं हो सकती।',
  'Tutar boş bırakılamaz.': 'राशि दर्ज करना ज़रूरी है।',
  'Geçerli bir para tutarı girin.': 'मान्य राशि दर्ज करें।',
  'Tutar biçimi anlaşılamadı.': 'राशि प्रारूप पहचाना नहीं जा सका।',
  'En fazla iki kuruş hanesi girilebilir.':
      'दशमलव के बाद अधिकतम दो अंक दर्ज किए जा सकते हैं।',
  'Değer': 'मान',
  'Lefferion Prime - MİZAN hata yapabilir. Lütfen vade, gecikme ve ödeme bilgilerini son kez kontrol edin.':
      'लेफ़रियन प्राइम - MİZAN गलतियाँ कर सकता है। कृपया आगे बढ़ने से पहले देय तिथियां, बकाया स्थिति और भुगतान जानकारी सत्यापित करें।',
  'Son ödeme bugün': 'आज देय',
  'Ocak': 'जनवरी',
  'Şubat': 'फ़रवरी',
  'Mart': 'मार्च',
  'Nisan': 'अप्रैल',
  'Mayıs': 'मई',
  'Haziran': 'जून',
  'Temmuz': 'जुलाई',
  'Ağustos': 'अगस्त',
  'Eylül': 'सितम्बर',
  'Ekim': 'अक्टूबर',
  'Kasım': 'नवंबर',
  'Aralık': 'दिसंबर',
  'Oca': 'जनवरी',
  'Şub': 'फ़रवरी',
  'Mar': 'मार्च',
  'Nis': 'अप्रैल',
  'May': 'मई',
  'Haz': 'जून',
  'Tem': 'जुलाई',
  'Ağu': 'अगस्त',
  'Eyl': 'सितम्बर',
  'Eki': 'अक्टूबर',
  'Kas': 'नवंबर',
  'Ara': 'खोजें',
  'Bildirim servisi bu platformda etkin değil.':
      'इस प्लेटफ़ॉर्म पर सूचना सेवा उपलब्ध नहीं है।',
  'Gider bildirimleri': 'खर्च सूचनाएं',
  'Ödeme bildirimleri': 'भुगतान सूचनाएं',
  'Günlük gider kaydı bildirimleri': 'दैनिक खर्च प्रविष्टि सूचनाएँ',
  'Tüm kayıt türlerinin son ödeme bildirimleri':
      'सभी रिकॉर्ड प्रकारों के लिए नियत तारीख की सूचनाएं',
  'Android dışında gerçek zamanlama yapılmaz.':
      'वास्तविक शेड्यूलिंग केवल Android पर उपलब्ध है।',
  'Bildirim izni kapalı.': 'सूचना अनुमति बंद है।',
  'Dakik bildirim izni kapalı. Saat ve dakika doğruluğu için izni açın.':
      'सटीक अलार्म की अनुमति बंद है। चुने गए घंटे और मिनट पर सूचना के लिए अनुमति चालू करें।',
  'Dakik bildirim izni verilmedi.': 'सटीक अलार्म की अनुमति नहीं दी गई थी।',
  'Bildirim izni kapalı. Yeni bildirimler oluşturulmadı.':
      'सूचना अनुमति बंद है. कोई नई सूचना नहीं बनाई गई।',
  'Dakik bildirim izni kapalı. Android mevcut dakik planları iptal eder; izin açıldığında plan yeniden kurulmalıdır.':
      'सटीक अलार्म अनुमति बंद है. Android मौजूदा सटीक शेड्यूल को रद्द करता है; अनुमति मिलने के बाद शेड्यूल को फिर से बनाया जाना चाहिए।',
  'Bildirim izni kapalı. Önce bildirim iznini açın.':
      'सूचना अनुमति बंद है. पहले सूचना अनुमति सक्षम करें।',
  'Dakik bildirim izni verilmedi. Test yaklaşık zamanda çalıştırılmayacak.':
      'सटीक अलार्म की अनुमति नहीं मिली। परीक्षण अनुमानित समय पर नहीं चलाया जाएगा।',
  'MİZAN bildirim testi': 'MİZAN सूचना परीक्षण',
  'Bu test, ayarlanan dakik bildirim sistemiyle oluşturuldu.':
      'यह परीक्षण कॉन्फ़िगर की गई सटीक सूचना प्रणाली के साथ बनाया गया था।',
  'Yedek kayıt doğrulanamadı.': 'बैकअप सत्यापित नहीं किया जा सका।',
  'Ana kayıt okunamadı; son sağlam yedek geri yüklendi.':
      'मुख्य डेटा फ़ाइल पढ़ी नहीं जा सकी; अंतिम सही बैकअप पुनर्स्थापित किया गया।',
  'Ana ve yedek kayıt dosyaları okunamadı. Dosyalar korunuyor.':
      'न तो मुख्य डेटा फ़ाइल और न ही बैकअप पढ़ा जा सका। फाइलों को सुरक्षित रखा गया है।',
  'MİZAN kullanıma hazır. İlk kişi veya kaydı ekleyebilirsin.':
      'MİZAN तैयार है. आरंभ करने के लिए अपना पहला व्यक्ति या रिकॉर्ड जोड़ें।',
  'Geçici kayıt doğrulanamadı.':
      'अस्थायी रूप से सहेजा गया डेटा सत्यापित नहीं हो सका।',
};
