// REVIEWED HINDI LOCALIZATION — NATURAL INDIA-ORIENTED PRODUCT COPY.
const Map<String, String> mizanHindiReports = <String, String>{
  'Ödemeleri, giderleri ve kalan yükü aynı filtreyle doğru ve ayrıntılı gösterir.':
      'एक ही फ़िल्टर से भुगतान, खर्च और शेष भुगतान दायित्वों को सही और विस्तृत रूप में दिखाता है।',
  'Ödemelere yapılan gider': 'किए गए भुगतान',
  'Normal giderler': 'सामान्य खर्च',
  'Kalan ödeme yükü': 'शेष भुगतान दायित्व',
  'Gecikmiş': 'अतिदेय',
  'Gelir ayrıntıları': 'आय का विवरण',
  'Serbest girilen gelir türleri ve seçili döneme düşen tutarları gösterilir.':
      'उपयोगकर्ता द्वारा दर्ज आय के प्रकार और चयनित अवधि में आने वाली राशियाँ दिखाई जाती हैं।',
  'Seçili dönemde gelir oluşmuyor.': 'चयनित अवधि में कोई आय नहीं है।',
  'Gelir bilgisi belirtilmemiş.': 'आय की कोई जानकारी नहीं दी गई।',
  'Gerçekleşen harcamaların dağılımı': 'वास्तविक खर्चों का वितरण',
  'Günlük harcamalar ve ödeme geçmişi ayrı kaynaklar olarak, en yüksek tutardan en düşüğe sıralanır.':
      'दैनिक खर्च और भुगतान इतिहास अलग-अलग स्रोतों के रूप में, सबसे अधिक से सबसे कम राशि तक क्रमबद्ध किए जाते हैं।',
  'Gerçekleşen ödeme ayrıntıları': 'किए गए भुगतानों का विवरण',
  'Kişi, kayıt, ödeme türü, tarih ve tutar birbirine karışmadan listelenir.':
      'व्यक्ति, रिकॉर्ड, भुगतान प्रकार, तिथि और राशि आपस में मिले बिना सूचीबद्ध किए जाते हैं।',
  'Seçili kapsamda gerçekleşen ödeme bulunmuyor.':
      'चयनित दायरे में कोई किया गया भुगतान नहीं मिला।',
  'Kalan ödeme yükünün dağılımı': 'शेष भुगतान दायित्वों का वितरण',
  'Toplam borcun tamamı değil, seçili döneme düşen sıradaki ödeme ve taksit tutarları gösterilir.':
      'पूरे कर्ज़ की शेष राशि नहीं, बल्कि चयनित अवधि में आने वाले अगले भुगतान और किस्त की राशियाँ दिखाई जाती हैं।',
  'Kalan ödeme ayrıntıları': 'शेष भुगतान का विवरण',
  'Seçili dönemde açık ödeme yükü bulunmuyor.':
      'चयनित अवधि में कोई खुला भुगतान दायित्व नहीं है।',
  'Gider dağılımı': 'खर्च का वितरण',
  'Normal giderler ile ödeme kayıtları aynı toplamda yer alır; kaynak türleri ayrı etiketlerle gösterilir.':
      'सामान्य खर्च और भुगतान रिकॉर्ड एक ही कुल में शामिल होते हैं, जबकि उनके स्रोत अलग-अलग लेबल से दिखाए जाते हैं।',
  'Seçili dönemde gider veya ödeme kaydı yok.':
      'चयनित अवधि में कोई खर्च या भुगतान रिकॉर्ड नहीं है।',
  'Bütün harcama ayrıntıları': 'सभी खर्चों का विवरण',
  'Her gün başlık olarak gösterilir. Başlığa dokununca günlük harcamalar ve ödemeler kendi bölümlerinde açılır.':
      'हर दिन को शीर्षक के रूप में दिखाया जाता है। दिन के शीर्षक पर टैप करने पर उसके खर्च और भुगतान अपने-अपने भाग में खुलते हैं।',
  'Seçili dönemde gider veya ödeme ayrıntısı bulunmuyor.':
      'चयनित अवधि में कोई खर्च या भुगतान विवरण नहीं मिला।',
  'Kişi kapsamı': 'शामिल लोग',
  'Tüm kişileri kapsa': 'सभी लोगों को शामिल करें',
  'Bütün kişilerin ödeme ve borç kayıtları rapora alınır.':
      'सभी लोगों के भुगतान और कर्ज़ रिकॉर्ड रिपोर्ट में शामिल किए जाते हैं।',
  'PDF hazırlanıyor.': 'PDF तैयार किया जा रहा है।',
  'MİZAN PDF raporunu kaydet': 'MİZAN PDF रिपोर्ट सहेजें',
  'PDF raporu kaydedildi.': 'PDF रिपोर्ट सहेजी गई।',
  'PDF raporu kaydedilemedi': 'PDF रिपोर्ट सहेजी नहीं जा सकी',
  'PDF raporu paylaşılamadı': 'PDF रिपोर्ट साझा नहीं की जा सकी',
  'Normal gider ayrıntıları': 'सामान्य खर्चों का विवरण',
  'Ödeme ayrıntıları': 'भुगतान का विवरण',
  'Kalan ödeme yükü ayrıntıları': 'शेष भुगतान दायित्वों का विवरण',
  'Gecikmiş ödeme ayrıntıları': 'अतिदेय भुगतानों का विवरण',
  'Yaklaşan ödeme ayrıntıları': 'आगामी भुगतानों का विवरण',
  'Normal giderler ve ödemeler ayrı başlıklar altında kalır; yalnız toplam hesaplamada birleşir.':
      'सामान्य खर्च और भुगतान अलग-अलग शीर्षकों के अंतर्गत रहते हैं और केवल कुल की गणना में जोड़े जाते हैं।',
  'Seçili döneme taşınan gecikmiş kayıtlar ile dönemin açık ödeme yükü ayrıntılı gösterilir.':
      'चयनित अवधि में आगे लाए गए अतिदेय रिकॉर्ड और उस अवधि के खुले भुगतान दायित्व विस्तार से दिखाए जाते हैं।',
  'Gecikmiş tutar, açık ve ödenmemiş dönemlerin toplamıdır.':
      'अतिदेय राशि खुली और भुगतान न की गई अवधियों का कुल है।',
  'Raporun referans gününden itibaren önümüzdeki 7 gün içinde vadesi kalan açık kayıtlar gösterilir.':
      'रिपोर्ट की संदर्भ तिथि से अगले 7 दिनों में देय खुले रिकॉर्ड दिखाए जाते हैं।',
  'Seçili kapsamda ayrıntı bulunmuyor.': 'चयनित दायरे में कोई विवरण नहीं मिला।',
  'Tüm kişiler': 'सभी लोग',
  'Rapor kapsamı': 'रिपोर्ट का दायरा',
  'Dönem ve kişi filtresi ekrandaki verilerle PDF’de birebir aynıdır.':
      'अवधि और व्यक्ति फ़िल्टर स्क्रीन तथा PDF में बिल्कुल समान हैं।',
  'Tüm kayıt geçmişi': 'पूरा रिकॉर्ड इतिहास',
  'Kayıtlı ay bulunmuyor': 'कोई सहेजा गया महीना नहीं मिला',
  'Kayıtlı yıl bulunmuyor': 'कोई सहेजा गया वर्ष नहीं मिला',
  'Güncel ay her zaman açılır; geçmişte kayıt, ödeme, gider veya gelir bulunan aylar ayrıca seçilebilir.':
      'चालू महीना हमेशा उपलब्ध रहता है; जिन पिछले महीनों में रिकॉर्ड, भुगतान, खर्च या आय है, उन्हें भी चुना जा सकता है।',
  'Güncel yıl her zaman açılır; kayıt bulunan geçmiş yıllar ayrıca seçilebilir.':
      'चालू वर्ष हमेशा उपलब्ध रहता है; जिन पिछले वर्षों में रिकॉर्ड हैं, उन्हें भी चुना जा सकता है।',
  'İlk kayıttan bugüne kadar bütün ödeme, gider ve gelir hareketleri kapsanır.':
      'पहले रिकॉर्ड से आज तक के सभी भुगतान, खर्च और आय लेन-देन शामिल किए जाते हैं।',
  'Kalan kayıt durumu (opsiyonel)': 'शेष रिकॉर्ड की स्थिति (वैकल्पिक)',
  'Tüm durumlar': 'सभी स्थितियाँ',
  'Gider kayıtlarında kişi alanı bulunmadığı için giderler seçili dönem kapsamında ve kişi filtresinden bağımsız hesaplanır.':
      'खर्च रिकॉर्ड में व्यक्ति का फ़ील्ड नहीं होता, इसलिए खर्च चयनित अवधि के लिए व्यक्ति फ़िल्टर से स्वतंत्र रूप से गिने जाते हैं।',
  'Kayıtlı yılı seç': 'सहेजा गया वर्ष चुनें',
  'Kayıtlı ayı seç': 'सहेजा गया महीना चुनें',
  'Gelir ve net durum': 'आय और शुद्ध स्थिति',
  'Gelirden gerçekleşen ödemeler ve giderler sırayla düşülür.':
      'किए गए भुगतान और खर्च क्रम से आय में से घटाए जाते हैं।',
  'PDF raporu': 'PDF रिपोर्ट',
  'Aynı raporu kaydedebilir veya WhatsApp dahil paylaşım menüsüne gönderebilirsin.':
      'आप इसी रिपोर्ट को सहेज सकते हैं या WhatsApp सहित शेयर मेनू में भेज सकते हैं।',
  'PDF hazırlanıyor': 'PDF तैयार किया जा रहा है',
  'PDF indir': 'PDF डाउनलोड करें',
  'PDF paylaş': 'PDF साझा करें',
  'Seçili dönem gider özeti': 'चयनित अवधि के खर्चों का सारांश',
  'Bütün harcamalar, normal giderler ile banka, şahıs, fatura, abonelik, kira ve taksit ödemelerinin toplamıdır.':
      'सभी खर्चों में सामान्य खर्च तथा बैंक कर्ज़, व्यक्तिगत कर्ज़, बिल, सदस्यता, किराया और किस्त के भुगतान शामिल हैं।',
  'Normal giderler ile banka, şahıs, fatura, abonelik, kira ve taksit ödemelerine yapılan giderlerin toplamıdır. Gelir ayrı gösterilir.':
      'यह सामान्य खर्च तथा बैंक कर्ज़, व्यक्तिगत कर्ज़, बिल, सदस्यता, किराया और किस्त के भुगतानों का कुल है। आय अलग दिखाई जाती है।',
  'Gelir sonrası net': 'आय के बाद शुद्ध राशि',
  'Kayıt bulunmuyor.': 'कोई रिकॉर्ड नहीं मिला।',
  'Daha fazla gider günü göster': 'खर्च वाले और दिन दिखाएँ',
  'Kişi bazında güncel kalan borç': 'व्यक्ति के अनुसार वर्तमान शेष कर्ज़',
  'Kişi ve kayıt türü başlıklarına dokunarak ayrıntıları açıp kapatabilirsiniz. Kayıt satırına dokununca tam kayıt detayı açılır.':
      'व्यक्ति और रिकॉर्ड प्रकार के शीर्षकों पर टैप करके विवरण खोले या बंद किए जा सकते हैं। रिकॉर्ड की पंक्ति पर टैप करने से उसका पूरा विवरण खुलता है।',
  'Toplam kalan': 'कुल शेष',
  'Hafta': 'सप्ताह',
  'Tüm zamanlar': 'पूरी अवधि',
  'PDF rapor sayfası görüntüye dönüştürülemedi.':
      'PDF रिपोर्ट के पृष्ठ को चित्र में नहीं बदला जा सका।',
  'Sayfa': 'पृष्ठ',
  'finans raporu': 'वित्तीय रिपोर्ट',
  'Kayıtlı kişi yok': 'कोई सहेजा गया व्यक्ति नहीं',
  'GÜN BAŞLIĞI': 'दिन का शीर्षक',
  'Rapor özeti': 'रिपोर्ट सारांश',
  'Ödeme kayıtları ve Giderler bölümü birbirine karıştırılmadan hesaplanır.':
      'भुगतान रिकॉर्ड और खर्च अनुभाग को आपस में मिलाए बिना गणना की जाती है।',
  'Ödemeler sonrası kalan gelir': 'भुगतान के बाद शेष आय',
  'Toplam gider sonrası net': 'कुल खर्च के बाद शुद्ध राशि',
  'Seçili dönemde kalan ödeme yükü': 'चयनित अवधि का शेष भुगतान दायित्व',
  'Gecikmiş ödeme yükü': 'अतिदेय भुगतान दायित्व',
  'Yaklaşan ödeme yükü': 'आगामी भुगतान दायित्व',
  'Gelir türleri seçili döneme düşen tekrar sayısına göre hesaplanır.':
      'आय के प्रकार चयनित अवधि में आने वाली आवृत्तियों की संख्या के अनुसार गिने जाते हैं।',
  'Seçili dönem ve kişi kapsamındaki ödeme geçmişi kayıt türüne göre ayrılır.':
      'चयनित अवधि और लोगों के दायरे का भुगतान इतिहास रिकॉर्ड प्रकार के अनुसार अलग किया जाता है।',
  'Her ödeme yalnız bağlı olduğu kişi ve kayıt altında gösterilir.':
      'हर भुगतान केवल उससे जुड़े व्यक्ति और रिकॉर्ड के अंतर्गत दिखाया जाता है।',
  'Gecikmiş kayıtlarda gösterilen taksit ve ana para tutarlarına işleyebilecek faizler, gecikme bedelleri ve diğer olası durum etkenleri dahil değildir.':
      'अतिदेय रिकॉर्ड में दिखाई गई किस्त और मूलधन की राशियों में संभावित ब्याज, विलंब शुल्क या अन्य लागू कारक शामिल नहीं हैं।',
  'Ödeme kayıtları': 'भुगतान रिकॉर्ड',
  'Normal giderler ve ödeme kayıtları aynı rapor toplamına dahil edilir; kaynakları birbirine karıştırılmadan ayrı renklerle gösterilir.':
      'सामान्य खर्च और भुगतान रिकॉर्ड एक ही रिपोर्ट के कुल में शामिल होते हैं और उनके स्रोत आपस में मिलाए बिना अलग-अलग रंगों में दिखाए जाते हैं।',
  'Seçili dönemde gider veya ödeme kaydı bulunmuyor.':
      'चयनित अवधि में कोई खर्च या भुगतान रिकॉर्ड नहीं है।',
  'Günler başlık olarak gösterilir; her günün normal harcamaları ve ödemeleri kendi bölümünde, satır taşması olmadan listelenir.':
      'दिन शीर्षकों के रूप में दिखाए जाते हैं; हर दिन के सामान्य खर्च और भुगतान अपने-अपने भाग में बिना पाठ कटे सूचीबद्ध होते हैं।',
  'Seçili dönemde gider ayrıntısı bulunmuyor.':
      'चयनित अवधि में कोई खर्च विवरण नहीं मिला।',
  'Toplam borcun tamamı değil, seçili döneme düşen sıradaki ödeme/taksit tutarları gösterilir.':
      'पूरे कर्ज़ की शेष राशि नहीं, बल्कि चयनित अवधि में आने वाले अगले भुगतान या किस्त की राशि दिखाई जाती है।',
  'Vade, kişi, kayıt türü, gecikme süresi ve sıradaki ödeme tutarı birlikte sunulur.':
      'देय तिथि, व्यक्ति, रिकॉर्ड प्रकार, देरी की अवधि और अगली भुगतान राशि एक साथ दिखाई जाती है।',
  'Seçili kişilerin bütün açık kayıtları, dönem filtresinden bağımsız güncel bakiye olarak sunulur.':
      'चयनित लोगों के सभी खुले रिकॉर्ड अवधि फ़िल्टर से स्वतंत्र, वर्तमान शेष राशि के रूप में दिखाए जाते हैं।',
  'Toplam güncel kalan borç': 'कुल वर्तमान शेष कर्ज़',
};
