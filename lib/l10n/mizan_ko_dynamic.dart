...
String translateKoreanReviewedDynamic(
  String source,
  KoreanDynamicTranslator translate,
) {
  ...
  var value = source;
  for (final entry in _phrases) value = value.replaceAll(entry.$1, entry.$2);
  return value;
}
...
final List<_KoreanPattern> _patterns = <_KoreanPattern>[
  ...
  _KoreanPattern(
    ...
    (m, t) => 'LEFFERION PRIME - MİZAN · ${_n(m[1]!)}페이지',
  ),
  ...
  _KoreanPattern(
    ...
    (m, t) => '${_newItems(m[1]!)} 추가, ${_updated(m[2]!)}${m[3]}.',
  ),
  _KoreanPattern(
    RegExp(r'^(.+) kayıt kimliği geçersiz veya tekrarlı\.$'),
    (m, t) => '기록 ID ${m[1]}이 올바르지 않거나 중복되었습니다.',
  ),
  ...
  _KoreanPattern(
    ...
    (m, t) => '${t(m[1]!)}은(는) 비워 둘 수 없습니다.',
  ),
  _KoreanPattern(
    RegExp(r'^(.+) en fazla (\d+) karakter olabilir\.$'),
    (m, t) => '${t(m[1]!)}은(는) 최대 ${_n(m[2]!)}자까지 입력할 수 있습니다.',
  ),
  _KoreanPattern(
    RegExp(r'^(.+) sıfırdan büyük olmalı\.$'),
    (m, t) => '${t(m[1]!)}은(는) 0보다 커야 합니다.',
  ),
  _KoreanPattern(
  ...
  _KoreanPattern(
    ...
    (m, t) => '${t(m[1]!)}은(는) 음수일 수 없습니다.',
  ),
  _KoreanPattern(
    RegExp(r'^(.+) pozitif tam sayı olmalı\.$'),
    (m, t) => '${t(m[1]!)}은(는) 양의 정수여야 합니다.',
  ),
  _KoreanPattern(
    RegExp(r'^(.+) sıfır veya pozitif tam sayı olmalı\.$'),
    (m, t) => '${t(m[1]!)}은(는) 0 또는 양의 정수여야 합니다.',
  ),
  ...
  _KoreanPattern(
    ...
    (m, t) => '${_newItems(m[1]!)} 추가됨. 기존 데이터는 유지되었습니다.',
  ),
  _KoreanPattern(
    RegExp(r'^Test (.+) için dakik olarak planlandı\.$'),
    (m, t) => '${m[1]} 테스트가 정확한 시간으로 예약되었습니다.',
  ),
  _KoreanPattern(
    RegExp(r'^Test planlanamadı: (.+)$'),
    (m, t) => '테스트를 예약할 수 없습니다: ${m[1]}',
  ),
  _KoreanPattern(
    RegExp(r'^(.+) planlanamadı: (.+)$'),
    (m, t) => '${t(m[1]!)}을(를) 예약할 수 없습니다: ${m[2]}',
  ),
  _KoreanPattern(
    RegExp(r'^(.+) kaydedilemedi: (.+)$'),
    (m, t) => '${t(m[1]!)}을(를) 저장할 수 없습니다: ${m[2]}',
  ),
  _KoreanPattern(
    RegExp(r'^(.+) oluşturulamadı: (.+)$'),
    (m, t) => '${t(m[1]!)}을(를) 생성할 수 없습니다: ${m[2]}',
  ),
  _KoreanPattern(
    RegExp(r'^(.+) paylaşılamadı: (.+)$'),
    (m, t) => '${t(m[1]!)}을(를) 공유할 수 없습니다: ${m[2]}',
  ),
  _KoreanPattern(
    RegExp(r'^(.+) birleştirilemedi: (.+)$'),
    (m, t) => '${t(m[1]!)}을(를) 병합할 수 없습니다: ${m[2]}',
  ),
];
...
class _KoreanPattern {
  const _KoreanPattern(this.regExp, this.builder);
  final RegExp regExp;
  final String Function(RegExpMatch, KoreanDynamicTranslator) builder;
}
