...
final List<_UrduPattern> _urduPatterns = <_UrduPattern>[
  ...
  _UrduPattern(
    ...
    (m, t) => 'اور دن دکھائیں (${_remaining(m[1]!)})',
  ),
  _UrduPattern(
    RegExp(r'^Daha fazla ödeme günü göster \((\d+) kaldı\)$'),
    (m, t) => 'ادائیگی کے مزید دن دکھائیں (${_remaining(m[1]!)})',
  ),
  _UrduPattern(
    RegExp(r'^Daha fazla gider günü göster \((\d+) kaldı\)$'),
    (m, t) => 'اخراجات کے مزید دن دکھائیں (${_remaining(m[1]!)})',
  ),
  _UrduPattern(
    RegExp(r'^Bu günden daha fazla göster \((\d+) kaldı\)$'),
    (m, t) => 'اس دن کے مزید ریکارڈ دکھائیں (${_remaining(m[1]!)})',
  ),
  _UrduPattern(
    RegExp(r'^(.+) için (\d+) gün kaldı$'),
    (m, t) => '${m[1]} کے لیے ${_remainingDays(m[2]!)}',
  ),
  _UrduPattern(
  ...
  _UrduPattern(
    ...
    (m, t) => '${m[1]} میں ${_days(m[2]!)} کی تاخیر ہے',
  ),
  _UrduPattern(
    RegExp(r'^Son alındı: (.+) · Planlanan (.+)$'),
    (m, t) => 'آخری وصولی: ${m[1]} · طے شدہ: ${m[2]}',
  ),
  _UrduPattern(
    RegExp(
    ...
    (m, t) =>
        'طے شدہ ${m[1]} مدت کو ${m[2]} کو موصول شدہ کے طور پر درج کیا گیا۔ مقررہ وصولی کا دن تبدیل نہیں ہوا۔',
  ),
  _UrduPattern(
    RegExp(r'^(.+) gerçek fatura tutarı$'),
    (m, t) => '${m[1]} کے بل کی حقیقی رقم',
  ),
  ...
  _UrduPattern(
    ...
    (m, t) => 'کیا ${m[1]} خرچ کا ریکارڈ حذف کر دیا جائے؟',
  ),
  _UrduPattern(
    RegExp(
      r'^(.+) kategorisi ve yalnız bu kategoriye bağlı giderler silinecek\.$',
    ),
    (m, t) => '${m[1]} زمرہ اور صرف اس سے منسلک اخراجات حذف کیے جائیں گے۔',
  ),
  _UrduPattern(
    RegExp(
    ...
    (m, t) =>
        '${m[1]} اور اس فرد سے منسلک تمام ریکارڈ حذف کیے جائیں گے۔ یہ کارروائی واضح تصدیق کے بعد ہی ہوگی۔',
  ),
  _UrduPattern(
    RegExp(r'^PDF raporu kaydedilemedi: (.+)$'),
    (m, t) => 'PDF رپورٹ محفوظ نہیں کی جا سکی: ${m[1]}',
  ),
  _UrduPattern(
    RegExp(r'^PDF raporu paylaşılamadı: (.+)$'),
    (m, t) => 'PDF رپورٹ شیئر نہیں کی جا سکی: ${m[1]}',
  ),
  _UrduPattern(
    RegExp(
  ...
  _UrduPattern(
    ...
    (m, t) => 'ادائیگی کی یاددہانی ${m[1]}',
  ),
  _UrduPattern(
    RegExp(r'^(.+) yeni, (.+) ilişki güncellendi(.*)\.$'),
    (m, t) =>
        '${_newItems(m[1]!)} شامل کیے گئے، ${_updatedLinks(m[2]!)}${m[3]}۔',
  ),
  _UrduPattern(
    RegExp(r'^(.+) kayıt kimliği geçersiz veya tekrarlı\.$'),
    (m, t) => 'ریکارڈ آئی ڈی ${m[1]} نامعتبر ہے یا دہرائی گئی ہے۔',
  ),
  _UrduPattern(RegExp(r'^(\d+) gün kaldı$'), (m, t) => _remainingDays(m[1]!)),
  _UrduPattern(
    RegExp(r'^(\d+) gün gecikmede$'),
    (m, t) => '${_days(m[1]!)} کی تاخیر',
  ),
  _UrduPattern(
    RegExp(r'^Ödeme (\d+) gün gecikti\.$'),
    (m, t) => 'ادائیگی میں ${_days(m[1]!)} کی تاخیر ہے۔',
  ),
  _UrduPattern(
    RegExp(r'^Son ödeme (.+)\.$'),
    (m, t) => 'آخری ادائیگی کی تاریخ: ${m[1]}۔',
  ),
  _UrduPattern(
    RegExp(r'^Ayın (\d+)\. günü$'),
    (m, t) => 'ماہ کا ${m[1]}واں دن',
  ),
  _UrduPattern(
    RegExp(r'^Her ayın (\d+)\. günü$'),
    (m, t) => 'ہر ماہ کا ${m[1]}واں دن',
  ),
  ...
  _UrduPattern(
    ...
    (m, t) => '${t(m[1]!)} خالی نہیں چھوڑا جا سکتا۔',
  ),
  _UrduPattern(
    RegExp(r'^(.+) en fazla (\d+) karakter olabilir\.$'),
    (m, t) => '${t(m[1]!)} میں زیادہ سے زیادہ ${m[2]} حروف ہو سکتے ہیں۔',
  ),
  _UrduPattern(
    RegExp(r'^(.+) sıfırdan büyük olmalı\.$'),
    (m, t) => '${t(m[1]!)} صفر سے زیادہ ہونا چاہیے۔',
  ),
  _UrduPattern(
  ...
  _UrduPattern(
    ...
    (m, t) => '${t(m[1]!)} منفی نہیں ہو سکتا۔',
  ),
  _UrduPattern(
    RegExp(r'^(.+) pozitif tam sayı olmalı\.$'),
    (m, t) => '${t(m[1]!)} مثبت صحیح عدد ہونا چاہیے۔',
  ),
  _UrduPattern(
    RegExp(r'^(.+) sıfır veya pozitif tam sayı olmalı\.$'),
    (m, t) => '${t(m[1]!)} صفر یا مثبت صحیح عدد ہونا چاہیے۔',
  ),
  ...
  _UrduPattern(
    ...
    (m, t) => '${_newItems(m[1]!)} شامل کیے گئے؛ موجودہ ڈیٹا محفوظ رہا۔',
  ),
  _UrduPattern(
    RegExp(r'^Test (.+) için dakik olarak planlandı\.$'),
    (m, t) => '${m[1]} کے لیے عین وقت کا ٹیسٹ مقرر کیا گیا۔',
  ),
  _UrduPattern(
    RegExp(r'^Test planlanamadı: (.+)$'),
    (m, t) => 'ٹیسٹ مقرر نہیں کیا جا سکا: ${m[1]}',
  ),
  _UrduPattern(
    RegExp(r'^(.+) planlanamadı: (.+)$'),
    (m, t) => '${t(m[1]!)} مقرر نہیں کیا جا سکا: ${m[2]}',
  ),
  _UrduPattern(
    RegExp(r'^(.+) kaydedilemedi: (.+)$'),
    (m, t) => '${t(m[1]!)} محفوظ نہیں کیا جا سکا: ${m[2]}',
  ),
  _UrduPattern(
    RegExp(r'^(.+) oluşturulamadı: (.+)$'),
    (m, t) => '${t(m[1]!)} بنایا نہیں جا سکا: ${m[2]}',
  ),
  _UrduPattern(
    RegExp(r'^(.+) paylaşılamadı: (.+)$'),
    (m, t) => '${t(m[1]!)} شیئر نہیں کیا جا سکا: ${m[2]}',
  ),
  _UrduPattern(
    RegExp(r'^(.+) birleştirilemedi: (.+)$'),
    (m, t) => '${t(m[1]!)} ضم نہیں کیا جا سکا: ${m[2]}',
  ),
];
