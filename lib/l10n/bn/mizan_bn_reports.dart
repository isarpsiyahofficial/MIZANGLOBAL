// REVIEWED BENGALI LOCALIZATION — NATURAL BANGLADESH/INDIA PRODUCT COPY.
const Map<String, String> mizanBengaliReports = <String, String>{
  'Ödemeleri, giderleri ve kalan yükü aynı filtreyle doğru ve ayrıntılı gösterir.':
      'একই ফিল্টারে পরিশোধ, খরচ এবং অবশিষ্ট পরিশোধের দায় সঠিক ও বিস্তারিতভাবে দেখায়।',
  'Ödemelere yapılan gider': 'পরিশোধে ব্যয়',
  'Normal giderler': 'সাধারণ খরচ',
  'Kalan ödeme yükü': 'অবশিষ্ট পরিশোধের দায়',
  'Gecikmiş': 'মেয়াদোত্তীর্ণ',
  'Gelir ayrıntıları': 'আয়ের বিবরণ',
  'Serbest girilen gelir türleri ve seçili döneme düşen tutarları gösterilir.':
      'ব্যবহারকারীর প্রবেশ করা আয়ের ধরন এবং নির্বাচিত সময়ের মধ্যে পড়ে থাকা পরিমাণ দেখায়।',
  'Seçili dönemde gelir oluşmuyor.': 'কোনো আয় নির্বাচিত সময়ের মধ্যে পড়ে না।',
  'Gelir bilgisi belirtilmemiş.': 'কোন আয় তথ্য প্রদান করা হয়.',
  'Gerçekleşen harcamaların dağılımı': 'বাস্তবায়িত খরচের বণ্টন',
  'Günlük harcamalar ve ödeme geçmişi ayrı kaynaklar olarak, en yüksek tutardan en düşüğe sıralanır.':
      'দৈনিক খরচ এবং অর্থপ্রদানের ইতিহাসকে পৃথক উৎস হিসাবে বিবেচনা করা হয় এবং সর্বোচ্চ থেকে সর্বনিম্ন পরিমাণে সাজানো হয়।',
  'Gerçekleşen ödeme ayrıntıları': 'সম্পন্ন পরিশোধের বিস্তারিত',
  'Kişi, kayıt, ödeme türü, tarih ve tutar birbirine karışmadan listelenir.':
      'ব্যক্তি, রেকর্ড, পরিশোধের ধরন, তারিখ ও পরিমাণ আলাদা রেখে তালিকাভুক্ত করা হয়।',
  'Seçili kapsamda gerçekleşen ödeme bulunmuyor.':
      'নির্বাচিত সুযোগের মধ্যে কোনো নথিভুক্ত পরিশোধ পাওয়া যায়নি।',
  'Kalan ödeme yükünün dağılımı': 'অসামান্য পরিশোধ ব্রেকডাউন',
  'Toplam borcun tamamı değil, seçili döneme düşen sıradaki ödeme ve taksit tutarları gösterilir.':
      'সম্পূর্ণ ঋণ নয়; নির্বাচিত সময়সীমায় পড়া পরবর্তী পরিশোধ ও কিস্তির পরিমাণ দেখানো হয়।',
  'Kalan ödeme ayrıntıları': 'অবশিষ্ট পরিশোধের বিস্তারিত',
  'Seçili dönemde açık ödeme yükü bulunmuyor.':
      'নির্বাচিত সময়সীমায় কোনো খোলা পরিশোধের দায় নেই।',
  'Gider dağılımı': 'খরচ ভাঙ্গন',
  'Normal giderler ile ödeme kayıtları aynı toplamda yer alır; kaynak türleri ayrı etiketlerle gösterilir.':
      'সাধারণ খরচ ও পরিশোধের রেকর্ড একই মোটে অন্তর্ভুক্ত থাকে; উৎসের ধরন আলাদা লেবেলে দেখানো হয়।',
  'Seçili dönemde gider veya ödeme kaydı yok.':
      'নির্বাচিত সময়ের মধ্যে কোন খরচ বা পরিশোধ রেকর্ড নেই।',
  'Bütün harcama ayrıntıları': 'সমস্ত খরচ বিবরণ',
  'Her gün başlık olarak gösterilir. Başlığa dokununca günlük harcamalar ve ödemeler kendi bölümlerinde açılır.':
      'প্রতিটি দিন একটি শিরোনাম হিসাবে দেখানো হয়. তাদের নিজস্ব বিভাগে দৈনিক খরচ এবং অর্থপ্রদান প্রসারিত করতে এটি আলতো চাপুন।',
  'Seçili dönemde gider veya ödeme ayrıntısı bulunmuyor.':
      'নির্বাচিত সময়ের মধ্যে কোনো খরচ বা অর্থপ্রদানের বিবরণ পাওয়া যায়নি।',
  'Kişi kapsamı': 'মানুষ অন্তর্ভুক্ত',
  'Tüm kişileri kapsa': 'সব মানুষ অন্তর্ভুক্ত',
  'Bütün kişilerin ödeme ve borç kayıtları rapora alınır.':
      'সমস্ত লোকের জন্য অর্থপ্রদান এবং ঋণের রেকর্ড প্রতিবেদনে অন্তর্ভুক্ত করা হয়েছে।',
  'PDF hazırlanıyor.': 'PDF প্রস্তুত করা হচ্ছে।',
  'MİZAN PDF raporunu kaydet': 'MİZAN PDF প্রতিবেদন সংরক্ষণ করুন',
  'PDF raporu kaydedildi.': 'PDF প্রতিবেদন সংরক্ষণ করা হয়েছে।',
  'PDF raporu kaydedilemedi': 'PDF প্রতিবেদন সংরক্ষণ করা যায়নি',
  'PDF raporu paylaşılamadı': 'PDF প্রতিবেদন শেয়ার করা যায়নি',
  'Normal gider ayrıntıları': 'নিয়মিত খরচের বিবরণ',
  'Ödeme ayrıntıları': 'পরিশোধ বিশদ',
  'Kalan ödeme yükü ayrıntıları': 'বকেয়া পরিশোধ বিশদ',
  'Gecikmiş ödeme ayrıntıları': 'মেয়াদোত্তীর্ণ পরিশোধের বিস্তারিত',
  'Yaklaşan ödeme ayrıntıları': 'আসন্ন পরিশোধের বিস্তারিত',
  'Normal giderler ve ödemeler ayrı başlıklar altında kalır; yalnız toplam hesaplamada birleşir.':
      'নিয়মিত খরচ এবং অর্থপ্রদান পৃথক শিরোনামের অধীনে থাকে এবং শুধুমাত্র মোট গণনা করার সময় একত্রিত হয়।',
  'Seçili döneme taşınan gecikmiş kayıtlar ile dönemin açık ödeme yükü ayrıntılı gösterilir.':
      'মেয়াদের বকেয়া পরিশোধ বাধ্যবাধকতা সহ নির্বাচিত সময়ের মধ্যে বাহিত ওভারডিউ রেকর্ড দেখায়।',
  'Gecikmiş tutar, açık ve ödenmemiş dönemlerin toplamıdır.':
      'ওভারডু অ্যামাউন্ট হল খোলা এবং অবৈতনিক মেয়াদের মোট।',
  'Raporun referans gününden itibaren önümüzdeki 7 gün içinde vadesi kalan açık kayıtlar gösterilir.':
      'প্রতিবেদনের রেফারেন্স তারিখের সাত দিনের মধ্যে খোলা রেকর্ড দেখায়।',
  'Seçili kapsamda ayrıntı bulunmuyor.':
      'নির্বাচিত সুযোগের মধ্যে কোন বিবরণ পাওয়া যায়নি.',
  'Tüm kişiler': 'সব মানুষ',
  'Rapor kapsamı': 'প্রতিবেদনের সুযোগ',
  'Dönem ve kişi filtresi ekrandaki verilerle PDF’de birebir aynıdır.':
      'সময়সীমা ও ব্যক্তি ফিল্টার পর্দা এবং PDF-এ হুবহু একই থাকে।',
  'Tüm kayıt geçmişi': 'সম্পূর্ণ রেকর্ড ইতিহাস',
  'Kayıtlı ay bulunmuyor': 'কোনো সংরক্ষিত মাস পাওয়া যায়নি',
  'Kayıtlı yıl bulunmuyor': 'কোনো সংরক্ষিত বছর পাওয়া যায়নি',
  'Güncel ay her zaman açılır; geçmişte kayıt, ödeme, gider veya gelir bulunan aylar ayrıca seçilebilir.':
      'বর্তমান মাস সর্বদা উপলব্ধ; রেকর্ড, পরিশোধ, খরচ, বা আয়ের সাথে আগের মাসগুলিও নির্বাচন করা যেতে পারে।',
  'Güncel yıl her zaman açılır; kayıt bulunan geçmiş yıllar ayrıca seçilebilir.':
      'বর্তমান বছর সবসময় পাওয়া যায়; রেকর্ড ধারণকারী বিগত বছর এছাড়াও নির্বাচন করা যেতে পারে.',
  'İlk kayıttan bugüne kadar bütün ödeme, gider ve gelir hareketleri kapsanır.':
      'প্রথম রেকর্ড থেকে আজ পর্যন্ত সমস্ত অর্থপ্রদান, খরচ এবং আয়ের কার্যকলাপ অন্তর্ভুক্ত।',
  'Kalan kayıt durumu (opsiyonel)': 'অসামান্য রেকর্ড স্থিতি (ঐচ্ছিক)',
  'Tüm durumlar': 'সব স্ট্যাটাস',
  'Gider kayıtlarında kişi alanı bulunmadığı için giderler seçili dönem kapsamında ve kişi filtresinden bağımsız hesaplanır.':
      'যেহেতু খরচের রেকর্ডে কোনো ব্যক্তি ক্ষেত্র নেই, তাই নির্বাচিত সময়ের জন্য খরচ গণনা করা হয় ব্যক্তি ফিল্টার থেকে স্বাধীনভাবে।',
  'Kayıtlı yılı seç': 'একটি সংরক্ষিত বছর নির্বাচন করুন',
  'Kayıtlı ayı seç': 'একটি সংরক্ষিত মাস নির্বাচন করুন',
  'Gelir ve net durum': 'আয় এবং নেট অবস্থান',
  'Gelirden gerçekleşen ödemeler ve giderler sırayla düşülür.':
      'রেকর্ডকৃত অর্থপ্রদান এবং খরচ ক্রমানুসারে আয় থেকে বাদ দেওয়া হয়।',
  'PDF raporu': 'PDF প্রতিবেদন',
  'Aynı raporu kaydedebilir veya WhatsApp dahil paylaşım menüsüne gönderebilirsin.':
      'একই প্রতিবেদন সংরক্ষণ করতে বা WhatsApp-সহ শেয়ার মেনুতে পাঠাতে পারেন।',
  'PDF hazırlanıyor': 'PDF প্রস্তুত করা হচ্ছে',
  'PDF indir': 'PDF ডাউনলোড করুন',
  'PDF paylaş': 'PDF শেয়ার করুন',
  'Seçili dönem gider özeti': 'নির্বাচিত সময়ের খরচের সারাংশ',
  'Bütün harcamalar, normal giderler ile banka, şahıs, fatura, abonelik, kira ve taksit ödemelerinin toplamıdır.':
      'সমস্ত খরচ হল নিয়মিত খরচ এবং ব্যাঙ্ক ঋণ, ব্যক্তিগত ঋণ, বিল, সাবস্ক্রিপশন, ভাড়া, এবং কিস্তি পরিশোধের সম্মিলিত মোট।',
  'Normal giderler ile banka, şahıs, fatura, abonelik, kira ve taksit ödemelerine yapılan giderlerin toplamıdır. Gelir ayrı gösterilir.':
      'ব্যাঙ্ক ঋণ, ব্যক্তিগত ঋণ, বিল, সাবস্ক্রিপশন, ভাড়া এবং কিস্তির প্রতি নিয়মিত খরচ এবং অর্থপ্রদানের সম্মিলিত মোট। আয় আলাদাভাবে দেখানো হয়।',
  'Gelir sonrası net': 'আয় বাদ দেওয়ার পর নিট অবস্থা',
  'Kayıt bulunmuyor.': 'কোন রেকর্ড পাওয়া যায়নি.',
  'Daha fazla gider günü göster': 'আরো খরচ দিন দেখান',
  'Kişi bazında güncel kalan borç': 'ব্যক্তি দ্বারা বর্তমান বকেয়া ঋণ',
  'Kişi ve kayıt türü başlıklarına dokunarak ayrıntıları açıp kapatabilirsiniz. Kayıt satırına dokununca tam kayıt detayı açılır.':
      'বিশদ বিবরণ প্রসারিত বা সঙ্কুচিত করতে ব্যক্তি এবং রেকর্ড-টাইপ শিরোনামে আলতো চাপুন। একটি রেকর্ড সারি এর সম্পূর্ণ বিবরণ খুলতে আলতো চাপুন।',
  'Toplam kalan': 'মোট বকেয়া',
  'Hafta': 'সপ্তাহ',
  'Tüm zamanlar': 'সব সময়',
  'PDF rapor sayfası görüntüye dönüştürülemedi.':
      'PDF প্রতিবেদনের পৃষ্ঠা ছবিতে রূপান্তর করা যায়নি।',
  'Sayfa': 'পাতা',
  'finans raporu': 'আর্থিক প্রতিবেদন',
  'Kayıtlı kişi yok': 'কোন সংরক্ষিত মানুষ',
  'GÜN BAŞLIĞI': 'দিনের শিরোনাম',
  'Rapor özeti': 'প্রতিবেদনের সারাংশ',
  'Ödeme kayıtları ve Giderler bölümü birbirine karıştırılmadan hesaplanır.':
      'অর্থপ্রদানের রেকর্ড এবং খরচ বিভাগ তাদের উত্স মিশ্রিত না করে গণনা করা হয়।',
  'Ödemeler sonrası kalan gelir': 'পরিশোধ পরে আয় বাকি',
  'Toplam gider sonrası net': 'মোট খরচের পর নিট অবস্থা',
  'Seçili dönemde kalan ödeme yükü':
      'নির্বাচিত সময়ের মধ্যে বকেয়া পরিশোধ বাধ্যবাধকতা',
  'Gecikmiş ödeme yükü': 'মেয়াদোত্তীর্ণ পরিশোধের দায়',
  'Yaklaşan ödeme yükü': 'আসন্ন পরিশোধের দায়',
  'Gelir türleri seçili döneme düşen tekrar sayısına göre hesaplanır.':
      'আয়ের প্রকারগুলি নির্বাচিত সময়ের মধ্যে সংঘটিত হওয়ার সংখ্যা দ্বারা গণনা করা হয়।',
  'Seçili dönem ve kişi kapsamındaki ödeme geçmişi kayıt türüne göre ayrılır.':
      'নির্বাচিত সময়ের মধ্যে অর্থপ্রদানের ইতিহাস এবং লোকেদের সুযোগ রেকর্ডের প্রকার দ্বারা পৃথক করা হয়।',
  'Her ödeme yalnız bağlı olduğu kişi ve kayıt altında gösterilir.':
      'প্রতিটি পরিশোধ শুধুমাত্র তার সংযুক্ত ব্যক্তি এবং রেকর্ডের অধীনে দেখানো হয়।',
  'Gecikmiş kayıtlarda gösterilen taksit ve ana para tutarlarına işleyebilecek faizler, gecikme bedelleri ve diğer olası durum etkenleri dahil değildir.':
      'ওভারডিউ রেকর্ডের জন্য দেখানো কিস্তি এবং মূল পরিমাণে সম্ভাব্য সুদ, বিলম্বের ফি, বা প্রযোজ্য হতে পারে এমন অন্যান্য কারণ অন্তর্ভুক্ত নয়।',
  'Ödeme kayıtları': 'পরিশোধ রেকর্ড',
  'Normal giderler ve ödeme kayıtları aynı rapor toplamına dahil edilir; kaynakları birbirine karıştırılmadan ayrı renklerle gösterilir.':
      'নিয়মিত খরচ এবং অর্থপ্রদানের রেকর্ডগুলি একই প্রতিবেদনে মোট অন্তর্ভুক্ত করা হয়েছে এবং তাদের উত্সগুলিকে মিশ্রিত না করে আলাদা রঙে দেখানো হয়েছে৷',
  'Seçili dönemde gider veya ödeme kaydı bulunmuyor.':
      'নির্বাচিত সময়ের মধ্যে কোন খরচ বা পরিশোধ রেকর্ড নেই।',
  'Günler başlık olarak gösterilir; her günün normal harcamaları ve ödemeleri kendi bölümünde, satır taşması olmadan listelenir.':
      'দিনগুলি শিরোনাম হিসাবে দেখানো হয়; প্রতিটি দিনের নিয়মিত খরচ এবং অর্থপ্রদান পাঠ্য ওভারফ্লো ছাড়াই তাদের নিজস্ব বিভাগে তালিকাভুক্ত করা হয়।',
  'Seçili dönemde gider ayrıntısı bulunmuyor.':
      'নির্বাচিত সময়ের মধ্যে কোনো খরচের বিবরণ পাওয়া যায়নি।',
  'Toplam borcun tamamı değil, seçili döneme düşen sıradaki ödeme/taksit tutarları gösterilir.':
      'নির্বাচিত সময়ের মধ্যে পরবর্তী অর্থপ্রদান/কিস্তির পরিমাণ দেখায়, সম্পূর্ণ ঋণ ব্যালেন্স নয়।',
  'Vade, kişi, kayıt türü, gecikme süresi ve sıradaki ödeme tutarı birlikte sunulur.':
      'শেষ তারিখ, ব্যক্তি, রেকর্ডের ধরন, ওভারডিউ সময়কাল এবং পরবর্তী অর্থপ্রদানের পরিমাণ একসাথে দেখানো হয়েছে।',
  'Seçili kişilerin bütün açık kayıtları, dönem filtresinden bağımsız güncel bakiye olarak sunulur.':
      'নির্বাচিত ব্যক্তিদের জন্য সমস্ত খোলা রেকর্ডগুলি পিরিয়ড ফিল্টার থেকে স্বাধীনভাবে বর্তমান ব্যালেন্স হিসাবে দেখানো হয়।',
  'Toplam güncel kalan borç': 'মোট বর্তমান বকেয়া ঋণ',
};
