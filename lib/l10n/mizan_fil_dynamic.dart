...
final List<_FilipinoPattern> _filipinoPatterns = <_FilipinoPattern>[
  _FilipinoPattern(
    ...
    (m, t) => 'Ulat ng MİZAN — ${t(m[1]!)}',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) finans raporu$'),
    (m, t) => 'Ulat sa pananalapi ng ${m[1]}',
  ),
  _FilipinoPattern(
    RegExp(r'^LEFFERION PRIME - MİZAN · Sayfa (\d+)$'),
    (m, t) => 'LEFFERION PRIME - MİZAN · Pahina ${_number(m[1]!)}',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) · devam$'),
    (m, t) => '${t(m[1]!)} · kasunod',
  ...
  _FilipinoPattern(
    ...
    (m, t) => 'Magpakita pa ng mga araw (${_remaining(m[1]!)})',
  ),
  _FilipinoPattern(
    RegExp(r'^Daha fazla ödeme günü göster \((\d+) kaldı\)$'),
    (m, t) => 'Magpakita pa ng mga araw ng pagbabayad (${_remaining(m[1]!)})',
  ),
  _FilipinoPattern(
    RegExp(r'^Daha fazla gider günü göster \((\d+) kaldı\)$'),
    (m, t) => 'Magpakita pa ng mga araw ng gastusin (${_remaining(m[1]!)})',
  ),
  _FilipinoPattern(
    RegExp(r'^Bu günden daha fazla göster \((\d+) kaldı\)$'),
    (m, t) => 'Magpakita pa ng mga tala sa araw na ito (${_remaining(m[1]!)})',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) için (\d+) gün kaldı$'),
    (m, t) => '${_remainingDays(m[2]!)} para sa ${m[1]}',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) bugün bekleniyor$'),
    (m, t) => 'Inaasahang matatanggap ngayon ang ${m[1]}',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) (\d+) gün gecikti$'),
    (m, t) => 'Overdue ng ${_days(m[2]!)} ang ${m[1]}',
  ),
  _FilipinoPattern(
    RegExp(r'^Son alındı: (.+) · Planlanan (.+)$'),
    (m, t) => 'Huling natanggap: ${m[1]} · Naka-iskedyul: ${m[2]}',
  ),
  _FilipinoPattern(
    RegExp(
    ...
    (m, t) =>
        'Ang nakaplanong period na ${m[1]} ay naitalang natanggap noong ${m[2]}. Hindi nagbago ang nakatakdang araw ng pagtanggap.',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) gerçek fatura tutarı$'),
    (m, t) => 'Aktuwal na halaga ng bayarin para sa ${m[1]}',
  ),
  _FilipinoPattern(
    RegExp(r'^Kalan tutar: (.+)$'),
    (m, t) => 'Natitirang halaga: ${m[1]}',
  ),
  _FilipinoPattern(
  ...
  _FilipinoPattern(
    ...
    (m, t) => 'Burahin ang tala ng gastusin na ${m[1]}?',
  ),
  _FilipinoPattern(
    RegExp(
    ...
    (m, t) =>
        'Buburahin ang kategoryang ${m[1]} at ang mga gastusing nakakabit lamang dito.',
  ),
  _FilipinoPattern(
    RegExp(
    ...
    (m, t) =>
        'Buburahin ang ${m[1]} at lahat ng talang nakakabit sa taong ito. Kailangan ng malinaw na kumpirmasyon para sa aksyong ito.',
  ),
  _FilipinoPattern(
    RegExp(r'^PDF raporu kaydedilemedi: (.+)$'),
    (m, t) => 'Hindi ma-save ang PDF report: ${m[1]}',
  ),
  _FilipinoPattern(
    RegExp(r'^PDF raporu paylaşılamadı: (.+)$'),
    (m, t) => 'Hindi ma-share ang PDF report: ${m[1]}',
  ),
  _FilipinoPattern(
    RegExp(
  ...
  _FilipinoPattern(
    ...
    (m, t) => 'Paalala sa pagbabayad ${_number(m[1]!)}',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) yeni, (.+) ilişki güncellendi(.*)\.$'),
    (m, t) =>
        '${_newItems(m[1]!)} ang idinagdag at ${_updatedLinks(m[2]!)}${m[3]}.',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) kayıt kimliği geçersiz veya tekrarlı\.$'),
    (m, t) => 'Hindi valid o duplicate ang record ID na ${m[1]}.',
  ),
  _FilipinoPattern(
  ...
  _FilipinoPattern(
    ...
    (m, t) => '${_days(m[1]!)} nang overdue',
  ),
  _FilipinoPattern(
    RegExp(r'^Ödeme (\d+) gün gecikti\.$'),
    (m, t) => 'Overdue ng ${_days(m[1]!)} ang bayad.',
  ),
  _FilipinoPattern(RegExp(r'^Son ödeme (.+)\.$'), (m, t) => 'Due ${m[1]}.'),
  _FilipinoPattern(
    RegExp(r'^Ayın (\d+)\. günü$'),
    (m, t) => 'Ika-${_number(m[1]!)} araw ng buwan',
  ),
  _FilipinoPattern(
    RegExp(r'^Her ayın (\d+)\. günü$'),
    (m, t) => 'Tuwing ika-${_number(m[1]!)} ng buwan',
  ),
  ...
  _FilipinoPattern(
    ...
    (m, t) => 'Hindi maaaring negatibo ang ${t(m[1]!)}.',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) pozitif tam sayı olmalı\.$'),
    (m, t) => 'Dapat positibong whole number ang ${t(m[1]!)}.',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) sıfır veya pozitif tam sayı olmalı\.$'),
    (m, t) => 'Dapat zero o positibong whole number ang ${t(m[1]!)}.',
  ),
  ...
  _FilipinoPattern(
    ...
    (m, t) =>
        '${_newItems(m[1]!)} ang idinagdag; nanatili ang kasalukuyang data.',
  ),
  _FilipinoPattern(
    RegExp(r'^Test (.+) için dakik olarak planlandı\.$'),
    (m, t) => 'Eksaktong naka-iskedyul ang test para sa ${m[1]}.',
  ),
  _FilipinoPattern(
    RegExp(r'^Test planlanamadı: (.+)$'),
    (m, t) => 'Hindi ma-iskedyul ang test: ${m[1]}',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) planlanamadı: (.+)$'),
    (m, t) => 'Hindi ma-iskedyul ang ${t(m[1]!)}: ${m[2]}',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) kaydedilemedi: (.+)$'),
    (m, t) => 'Hindi ma-save ang ${t(m[1]!)}: ${m[2]}',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) oluşturulamadı: (.+)$'),
    (m, t) => 'Hindi magawa ang ${t(m[1]!)}: ${m[2]}',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) paylaşılamadı: (.+)$'),
    (m, t) => 'Hindi ma-share ang ${t(m[1]!)}: ${m[2]}',
  ),
  _FilipinoPattern(
    RegExp(r'^(.+) birleştirilemedi: (.+)$'),
    (m, t) => 'Hindi mapagsama ang ${t(m[1]!)}: ${m[2]}',
  ),
];
