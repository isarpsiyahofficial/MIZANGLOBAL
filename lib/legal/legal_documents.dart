...
3. Exports and sharing. CSV backup export and import are available only to a verified permanent Google Play PRO purchase; temporary PRO from rewarded advertising or promotion codes does not unlock backup. A permanent-PRO CSV backup may contain a one-way fingerprint derived from Google Play purchase proof so the App can preserve purchase provenance without storing the Google account email or raw purchase token in the backup. The fingerprint alone never grants PRO; Google Play ownership remains authoritative. PDF reports are created only when the user requests them. If the user saves or shares an exported file through another application, storage provider or communication service, that destination handles the file under its own privacy terms. Exported files can contain sensitive financial information and should be protected appropriately.
...
5. Local data and backups. The App is intentionally serverless for user financial records. The publisher does not provide a cloud ledger or guaranteed remote backup. CSV backup export/import is a permanent Google Play PRO benefit and is not unlocked by temporary rewarded or promotion PRO. A backup purchase fingerprint is only supporting provenance and cannot independently create an entitlement; Google Play must report valid permanent ownership before backup restore is permitted. Users are responsible for the accuracy of records and for protecting backups appropriate to the importance of their data. Local data can be lost through device failure, operating-system behavior, user deletion, factory reset, storage corruption or uninstall without a usable backup.
